...
BATCH_OUTPUT_SCHEMA = f"""\
Respond with exactly one JSON array, one object per function. Use the address \
from each function header to populate the address field.
```json
[
  {{
    "address": "0x00401000",
