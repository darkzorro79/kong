 ...
 * Compiler: gcc
 *
 * Functions: 396 total, 355 analyzed, 40 skipped, 1 errors
 * Renamed:   8 | Confirmed: 347
 * LLM calls: 355
 * Duration:  15m 12.9s
 * Cost:      $6.6270
 * ============================================================ */
 ...
 * @brief ChaCha20 cipher context initialization: clears state, copies 256-bit key and 96-bit nonce, sets 'expand 32-byte k' constants and counter.
 * @confidence 92%
 * @classification crypto
 * @address 0x001093f0
 */

/* ChaCha20 cipher context initialization: clears state, copies 256-bit key and 96-bit nonce, sets
   magic constants and counter. */
 ...
 * @brief ChaCha20 stream cipher: 20 rounds (10 double-rounds) with quarter-round operations, generates 64-byte keystream blocks, XORs with data.
 * @confidence 95%
 * @classification crypto
 * @address 0x00109520
 */

/* ChaCha20 stream cipher. Implements 20 rounds (10 double-rounds) with quarter-round operations
   (add, XOR, rotate 16/12/8/7). Generates 64-byte keystream blocks, XORs with data, increments
   counter. */
 ...
 * @brief XZ backdoor: intercepts RSA_public_decrypt. When running as root and magic matches, decrypts and executes shellcode via ChaCha20
 * @confidence 95%
 * @classification crypto
 * @address 0x00109820
 */

/* XZ backdoor: intercepts RSA_public_decrypt. When running as root and magic matches, decrypts and
   executes shellcode via ChaCha20 */
 ...
 * @brief Public API entry for block buffer encoding, tail-calls implementation
 * @confidence 75%
 * @classification crypto
 * @address 0x0010a790
 */

/* Tail-call wrapper for block buffer encode implementation */
...
/* Wrapper for uncompressed block encoding via lzma_block_buffer_encode with null allocator */
 ...
 * @brief Updates index hash record statistics by accumulating VLI-encoded values and computing SHA-256 check hash
 * @confidence 82%
 * @classification crypto
 * @address 0x00110e80
 */

/* Updates index hash record statistics by accumulating VLI-encoded values and computing check hash
    */

void lzma_index_hash_record_update(long *param_1,long param_2,long param_3)

{
  ...
  *param_1 = *param_1 + (param_2 + 3U & 0xfffffffffffffffc);
  ...
  local_48 = param_2;
  local_40 = param_3;
  lzma_check_update(param_1 + 4,10,&local_48,0x10);
 ...
 * @brief Updates checksum: dispatches to CRC32 (type 1), CRC64 (type 4), or SHA-256 (type 10)
 * @confidence 92%
 * @classification crypto
 * @address 0x00114b50
 */

/* Updates checksum: dispatches to CRC32 (type 1), CRC64 (type 4), or SHA-256 (type 10) */

void lzma_check_update(long param_1,int param_2,undefined8 param_3,undefined8 param_4)
...
/* Finalizes integrity check by type: copies CRC32/CRC64 or calls SHA-256 finalize */
...
/* Software CRC32 computation using 8-way lookup tables with 8-byte chunk processing. */

uint lzma_crc32_generic(uint *param_1,byte *param_2,uint param_3)
 ...
 * @brief CRC-32 computation using CLMUL instructions with fold and Barrett reduction constants. Returns inverted CRC-32.
 * @confidence 92%
 * @classification crypto
 * @address 0x00114d20
 */
...
/* CRC-32 computation using CLMUL instructions (emulated via bit-by-bit polynomial multiplication).
   Uses fold and Barrett reduction constants. Returns inverted CRC-32. */

uint lzma_crc32_clmul(ulong param_1,ulong param_2,uint param_3)

{
  ...
  if (param_2 != 0) {
    ...
    auVar29[0] = clmul_shuffle_base_vector - auVar24[0];
    ...
    auVar28[0] = clmul_shuffle_base_vector - auVar24[0];
    ...
    if (param_2 < 0x11) {
      ...
      auVar27[0] = auVar29[0] + clmul_shuffle_base_vector;
      ...
      auVar24 = pshufb(auVar24,auVar27 ^ _clmul_xor_mask_vector);
      if (uVar26 < 0x11) {
        auVar28 = pshufb(auVar30,auVar28);
      }
      else {
        auVar30 = pshufb(auVar30,_clmul_xor_mask_vector ^ auVar28);
      ...
      auVar24 = pshufb(auVar24,auVar29 ^ _clmul_xor_mask_vector);
      ...
      if (pauVar1 != pauVar22) {
        ...
        auVar30 = pshufb(auVar27,_clmul_xor_mask_vector ^ auVar28);
...
/* Software CRC64 computation using 4-way lookup tables with word-at-a-time processing. */

ulong lzma_crc64_generic(uint *param_1,byte *param_2,ulong param_3)
 ...
 * @brief CRC-64 computation using CLMUL (carry-less multiplication) with SIMD, Barrett reduction. Returns 64-bit CRC with final NOT.
 * @confidence 92%
 * @classification crypto
 * @address 0x00115030
 */
...
/* CRC-64 computation using CLMUL (carry-less multiplication) with SIMD. Handles unaligned data,
   processes 16-byte chunks with fold constants, performs Barrett reduction. Returns 64-bit CRC with
   final NOT. */

ulong lzma_crc64_clmul(ulong param_1,ulong param_2,ulong param_3)

{
  ...
  if (param_2 != 0) {
    ...
    auVar27[0] = clmul_shuffle_base_vector - auVar24[0];
    ...
    auVar28[0] = clmul_shuffle_base_vector - auVar25[0];
    ...
    if (param_2 < 0x11) {
      ...
      auVar26[0] = auVar27[0] + clmul_shuffle_base_vector;
      ...
      auVar24 = pshufb(auVar24,auVar26 ^ _clmul_xor_mask_vector);
      if (uVar23 < 0x11) {
        auVar25 = pshufb(auVar25,auVar28);
      }
      else {
        auVar25 = pshufb(auVar25,_clmul_xor_mask_vector ^ auVar28);
      ...
      if (pauVar1 != pauVar21) {
        ...
        auVar24 = pshufb(auVar26,_clmul_xor_mask_vector ^ auVar28);
 ...
 * @brief SHA-256 block compression function: byte-swaps input, performs 64 rounds with standard round constants, message schedule expansion.
 * @confidence 95%
 * @classification crypto
 * @address 0x00115250
 */

/* SHA-256 block compression function. Byte-swaps input to big-endian, performs 64 rounds with
   standard round constants, message schedule expansion with sigma0/sigma1, adds result to 8-word
   state. */
 ...
 * @brief Initializes SHA-256 state with standard initial hash values from global constants
 * @confidence 92%
 * @classification crypto
 * @address 0x00116380
 */
...
/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */
/* Initializes SHA-256 state from global initial hash values */

void lzma_sha256_init(long param_1)
...
/**
 * @name  sha256_update
 * @brief SHA-256 update: feeds input data into 64-byte internal buffer, calls block compression when full. Handles streaming input.
 * @confidence 92%
 * @classification crypto
 * @address 0x001163b0
 */
...
void sha(undefined8 *param_1,ulong param_2,long param_3)
 ...
 * @brief SHA-256 finalization: pads message to 64-byte boundary, appends big-endian bit length, processes final block, byte-swaps 8-word hash output.
 * @confidence 95%
 * @classification crypto
 * @address 0x001164f0
 */

/* SHA-256 finalization: applies padding, appends bit length, processes final block, byte-swaps
   output hash. */
 ...
 * @brief Applies LZMA coder step via function pointer, then delegates to next coder in chain
 * @confidence 80%
 * @classification crypto
 * @address 0x00116c00
 */

/* Applies LZMA coder step via function pointer at coder[3], then delegates to next coder */
 ...
 * @brief LZMA2 encode step: returns LZMA_STREAM_END if state==1, otherwise calls sub-encoder
 * @confidence 80%
 * @classification crypto
 * @address 0x0011a320
 */

/* LZMA2 encode step: returns LZMA_STREAM_END if state==1, otherwise calls sub-encoder */
...

{
  ...
  if (*(int *)(param_2 + 0x68) != 1) {
    uVar1 = lzma_lzma_encoder_encode();
    return uVar1;
  }
  return 8;
}
 ...
 * @brief LZMA optimal parsing engine using dynamic programming. Evaluates literals, short reps, rep matches (rep0-rep3), and normal matches by computing bit costs via probability price tables. Maintains an optimal path array (at coder+0x10ecc region) with states tracking price, back distance, length, and previous optimal node. Updates position price tables for distance slots and alignment bits. Traces back the optimal path to return the best distance (*back_res) and length (*len_res). Returns -1/1 for literal encoding when no profitable match is found. Uses match finder (lzma_mf_find_best_match) and literal price calculation (lzma_literal_price). Handles rep-match-after-literal and match-after-literal combined sequences for better compression ratio.
 * @confidence 82%
 * @classification crypto
 * @address 0x0011add0
 */

/* LZMA optimal parsing with dynamic programming. Evaluates literals, short reps, rep matches,
   normal matches computing bit costs via probability price tables. Traces back optimal path for
   best distance/length. */

void lzma_lzma_optimal_match(long param_1,long *param_2,int *param_3,uint *param_4,uint param_5)

{
  ...
  if (*(int *)((long)param_2 + 0x1c) == 0) {
    if (0x7f < *(uint *)(param_1 + 0x10e7c)) {
      ...
      do {
        ...
        if (*(int *)(param_1 + 0x10e78) != 0) {
          do {
            ...
            do {
              uVar39 = uVar41 >> 1;
              iVar14 = iVar14 + (uint)(byte)(&probability_price_table)
      ...
      do {
        ...
        do {
          ...
          iVar49 = iVar49 + (uint)(byte)(&probability_price_table)
                                        [((uint)*(ushort *)
                                                 (param_1 + uVar28 * 2 +
    ...
    uVar41 = lzma_mf_find_best_match(param_2,&local_5c,param_1 + 0x2dc);
  }
  else {
...
LAB_0011c37f:
    ...
    if ((1 < uVar41 || cVar6 == cVar7) || 1 < local_134) {
      ...
      *(uint *)(param_1 + 0x10ecc) = uVar39;
      ...
      iVar14 = lzma_literal_price(param_1,param_5,*(undefined1 *)(lVar43 - 2),6 < uVar39,cVar7,
                                  cVar6);
      lVar43 = param_1 + (ulong)uVar39 * 2;
      *(undefined4 *)(param_1 + 0x10f10) = 0xffffffff;
      *(undefined1 *)(param_1 + 0x10efc) = 0;
      bVar8 = (&probability_price_table)[uVar15 & 0xfff];
      bVar9 = (&probability_price_table)[(uVar15 ^ 0x7f) & 0xfff];
      *(uint *)(param_1 + 0x10f08) = iVar14 + (uint)bVar8;
      uVar38 = *(ushort *)(lVar43 + 0x6d04) >> 4;
      iVar49 = (uint)(byte)(&probability_price_table)[uVar38 ^ 0x7f] + (uint)bVar9;
      if ((cVar6 == cVar7) &&
         (uVar39 = (uint)(byte)(&probability_price_table)[*(ushort *)(lVar43 + 0x6d1c) >> 4] +
                   (uint)(byte)(&probability_price_table)[*(ushort *)(lVar26 + 0x6d64) >> 4] + iVar49,
        ...
        *(undefined8 *)(param_1 + 0x10ef0) = *(undefined8 *)(param_1 + 0x2d4);
        ...
        do {
          ...
          if (1 < uVar29) {
            ...
            if (lVar18 == 0) {
              iVar33 = (uint)(byte)(&probability_price_table)[*(ushort *)(lVar26 + 0x6d64) >> 4 ^ 0x7f] +
                       (uint)(byte)(&probability_price_table)[uVar39];
            }
            else {
              uVar15 = (uint)(*(ushort *)(lVar43 + 0x6d34) >> 4);
              if (iVar14 == 1) {
                iVar33 = (uint)(byte)(&probability_price_table)[uVar15] +
                         (uint)(byte)(&probability_price_table)[uVar39 ^ 0x7f];
              }
              else {
                iVar33 = (uint)(byte)(&probability_price_table)
                                     [(2U - iVar14 & 0x7ff ^ (uint)*(ushort *)(lVar43 + 0x6d4c)) >>
                                      4] +
                         (uint)(byte)(&probability_price_table)[uVar39 ^ 0x7f] +
                         (uint)(byte)(&probability_price_table)[uVar15 ^ 0x7f];
              }
            }
            uVar39 = uVar29 - 2;
        ...
        if (uVar29 <= uVar41) {
          ...
          bVar8 = (&probability_price_table)[uVar38];
          uVar36 = uVar28;
          do {
            ...
            if (uVar39 < 0x80) {
              ...
              if (uVar39 < 0x80000) {
                uVar25 = (ulong)((byte)(&log2_slot_table)[uVar39 >> 6] + 0xc);
              }
              else if ((int)uVar39 < 0) {
                uVar25 = (ulong)((byte)(&log2_slot_table)[uVar39 >> 0x1e] + 0x3c);
              }
              else {
                uVar25 = (ulong)((byte)(&log2_slot_table)[uVar39 >> 0x12] + 0x24);
              }
...
LAB_0011b228:
          local_10c = lzma_mf_find_best_match(param_2,param_1 + 0xb6c,param_1 + 0x2dc);
          uVar29 = *(uint *)(param_2 + 0xc);
...
LAB_0011b380:
            ...
            *(int *)(lVar26 + 0x10ee8) = (int)uVar36;
            ...
            iVar49 = lzma_literal_price(param_1,uVar15,*(char *)((long)psVar17 - 1));
            local_114 = local_138 + 1;
            uVar46 = (ulong)local_114;
            uVar39 = iVar49 + iVar14 + (uint)(byte)(&probability_price_table)[uVar31 & 0xfff];
            ...
            iVar14 = iVar14 + (uint)(byte)(&probability_price_table)[(uVar31 ^ 0x7f) & 0xfff];
            iVar49 = (uint)(byte)(&probability_price_table)[uVar38 ^ 0x7f] + iVar14;
            uVar31 = uVar41;
            if ((char)sVar12 == cVar6) {
              lVar19 = param_1 + uVar46 * 0x2c;
              if ((local_138 <= *(uint *)(lVar19 + 0x10ee0)) || (*(int *)(lVar19 + 0x10ee4) != 0)) {
                uVar39 = (uint)(byte)(&probability_price_table)[*(ushort *)(lVar18 + 0x6d1c) >> 4] +
                         (uint)(byte)(&probability_price_table)[*(ushort *)(lVar26 + 0x6d64) >> 4] + iVar49;
                lVar19 = param_1 + uVar46 * 0x2c;
...
LAB_0011b553:
                if (1 < uVar30 - 1) {
                  ...
                  bVar8 = (&probability_price_table)[*(ushort *)(lVar3 + 0x6b84) >> 4 ^ 0x7f];
                  bVar9 = (&probability_price_table)[*(ushort *)(lVar19 + 0x6d04) >> 4 ^ 0x7f];
                  if (local_134 < uVar24) {
                    uVar32 = local_134 + 1;
                    uVar22 = ~local_134;
                  ...
                  uVar39 = (uint)bVar8 +
                           ...
                           (uint)(byte)(&probability_price_table)[*(ushort *)(lVar19 + 0x6d1c) >> 4] +
                           (uint)bVar9 +
                           (uint)(byte)(&probability_price_table)[*(ushort *)(lVar3 + 0x6d64) >> 4 ^ 0x7f];
...
LAB_0011b7e3:
                ...
                if (lVar19 == 0) {
                  iVar47 = (uint)(byte)(&probability_price_table)[*(ushort *)(lVar26 + 0x6d64) >> 4 ^ 0x7f] +
                           (uint)(byte)(&probability_price_table)[uVar32];
                }
                else {
                  uVar22 = (uint)(*(ushort *)(lVar18 + 0x6d34) >> 4);
                  if (iVar33 == 1) {
                    iVar47 = (uint)(byte)(&probability_price_table)[uVar22] +
                             (uint)(byte)(&probability_price_table)[uVar32 ^ 0x7f];
                  }
                  else {
                    iVar47 = (uint)(byte)(&probability_price_table)
                                         [(2U - iVar33 & 0x7ff ^ (uint)*(ushort *)(lVar18 + 0x6d4c))
                                          >> 4] +
                             (uint)(byte)(&probability_price_table)[uVar32 ^ 0x7f] +
                             (uint)(byte)(&probability_price_table)[uVar22 ^ 0x7f];
                  }
                }
...
LAB_0011b951:
  if (1 < iVar42 - 1U) {
    ...
    bVar8 = (&probability_price_table)
            [*(ushort *)
              (param_1 + 4 + ((ulong)(uVar22 & uVar32) + 0x35c0 + (ulong)(uVar30 + 0xb) * 0x10) * 2)
             >> 4];
    iVar16 = lzma_literal_price(param_1,uVar22,*(char *)((long)psVar17 + (ulong)(iVar16 - 1)),1,
    ...
    bVar9 = (&probability_price_table)[*(ushort *)(lVar4 + 0x6b84) >> 4 ^ 0x7f];
    bVar10 = (&probability_price_table)[*(ushort *)(lVar3 + 0x6d04) >> 4 ^ 0x7f];
    ...
    uVar22 = iVar47 + (uint)bVar10 + (uint)(byte)(&probability_price_table)[*(ushort *)(lVar3 + 0x6d1c) >> 4] +
                      iVar13 + (uint)bVar8 + (uint)bVar9 +
                      (uint)(byte)(&probability_price_table)[*(ushort *)(lVar4 + 0x6d64) >> 4 ^ 0x7f] +
  ...
  if (uVar39 <= local_10c) {
    ...
    do {
      ...
      if (uVar24 < 0x80) {
        ...
        if (uVar24 < 0x80000) {
          uVar27 = (ulong)((byte)(&log2_slot_table)[uVar24 >> 6] + 0xc);
        }
        else if ((int)uVar24 < 0) {
          uVar27 = (ulong)((byte)(&log2_slot_table)[uVar24 >> 0x1e] + 0x3c);
        }
        else {
          uVar27 = (ulong)((byte)(&log2_slot_table)[uVar24 >> 0x12] + 0x24);
        }
...
LAB_0011be78:
          ...
          if (1 < uVar35) {
            ...
            uVar50 = uVar23 & uVar2;
            bVar9 = (&probability_price_table)
                    [*(ushort *)(param_1 + 4 + ((ulong)uVar50 + 0x35c0 + (ulong)uVar30 * 0x10) * 2)
                     >> 4];
            iVar49 = lzma_literal_price(param_1,uVar2,*(char *)((long)psVar17 + (ulong)(uVar31 - 1))
                                        ,1,*(char *)((long)psVar17 + uVar25 + ~(ulong)uVar24),
            ...
            bVar10 = (&probability_price_table)[*(ushort *)(lVar18 + 0x6b84) >> 4 ^ 0x7f];
            bVar11 = (&probability_price_table)[*(ushort *)(lVar26 + 0x6d04) >> 4 ^ 0x7f];
            ...
            uVar31 = iVar49 + (uint)bVar11 +
                              (uint)(byte)(&probability_price_table)[*(ushort *)(lVar26 + 0x6d1c) >> 4] +
                              (uint)bVar9 + (uint)bVar10 +
                              (uint)(byte)(&probability_price_table)[*(ushort *)(lVar18 + 0x6d64) >> 4 ^ 0x7f]
                              + uVar32 +
                              *(int *)(param_1 + 0xba34 +
...
/* Reads from the head of the output queue, copies data via lzma_bufcpy, removes completed nodes */

undefined4
lzma_outq_read(long *param_1,undefined8 param_2,undefined8 param_3,undefined8 param_4,
...

{
  ...
  if ((*(char *)(lVar2 + 0x28) != '\0') && (*(ulong *)(lVar2 + 0x18) <= (ulong)param_1[2])) {
    ...
    lzma_outq_remove_head(param_1,param_2);
    param_1[2] = 0;
    return uVar1;
  }
  return 0;
...
/**
 * @name  lzma_block_buffer_encode
 * @brief Encodes LZMA block into buffer. Tries compressed first, falls back to uncompressed if larger. Writes header, data, padding, check.
 * @confidence 90%
 * @classification io
 * @address 0x0010a1b0
 */

/* Encodes LZMA block into buffer. Tries compressed encoding first, falls back to uncompressed if
   larger. Writes block header, data, padding, and check value (CRC/SHA). Validates all parameters.
    */
...
LAB_0010a6ab:
    ...
    if (__n_00 == 0) {
      ...
      lzma_check_update(&local_a8,param_1[2],param_3,param_4);
...
/* Single-call raw LZMA encoding from input to output buffer with filter chain initialization and
   cleanup. */
 ...
 * @brief Encodes complete LZMA stream into buffer: stream header, block-encode, index, stream footer.
 * @confidence 90%
 * @classification io
 * @address 0x0010bc70
 */

/* Encodes a complete LZMA stream into a buffer: writes stream header, block-encodes data, creates
   index, writes stream footer. Validates parameters and check type support. */
 ...
 * @brief Single-call LZMA block decoding from buffer: validates params, creates temp decoder, runs with LZMA_FINISH.
 * @confidence 90%
 * @classification io
 * @address 0x0010ee80
 */

/* Single-call LZMA block decoding: validates params, creates temp decoder, runs with LZMA_FINISH,
   restores positions on failure. */
 ...
 * @brief LZMA raw buffer decode: validates parameters, initializes decoder chain, processes in single pass, verifies completion.
 * @confidence 90%
 * @classification io
 * @address 0x00110360
 */

/* LZMA raw buffer decode: validates parameters, initializes a decoder chain, processes compressed
   data in a single pass, verifies all input consumed and output filled, then cleans up. Returns
   LZMA_OK (0) on success or error codes (9=DATA_ERROR, 10=BUF_ERROR, 11=PROG_ERROR). */

int lzma_raw_buffer_decode
              (undefined8 param_1,undefined8 param_2,long param_3,ulong *param_4,ulong param_5,
              long param_6,ulong *param_7,ulong param_8)

{
  ...
  if ((param_3 != 0) && (param_4 != (ulong *)0x0)) {
    iVar3 = 0xb;
    if (param_5 < *param_4) goto LAB_001104a4;
    if ((param_6 != 0) && (param_7 != (ulong *)0x0)) {
      if (*param_7 <= param_8) {
        local_98 = ZEXT816(0xffffffffffffffff) << 0x40;
        local_88 = (undefined1  [16])0x0;
        local_78 = (undefined1  [16])0x0;
        local_68 = (undefined1  [16])0x0;
        local_58 = (undefined1  [16])0x0;
        iVar3 = lzma_raw_decoder_init(local_98,param_2,param_1);
        if (iVar3 == 0) {
          uVar1 = *param_4;
          uVar2 = *param_7;
          iVar4 = (*(code *)local_88._8_8_)
                            (local_98._0_8_,param_2,param_3,param_4,param_5,param_6,param_7,param_8,
                             3);
          if (iVar4 != 1) {
            iVar3 = iVar4;
            if (((iVar4 == 0) && (iVar3 = 10, *param_4 == param_5)) &&
               (iVar3 = 9, *param_7 == param_8)) {
              local_a0 = 0;
              (*(code *)local_88._8_8_)
                        (local_98._0_8_,param_2,param_3,param_4,param_5,&local_41,&local_a0,1,3);
              iVar3 = (local_a0 == 1) + 9;
            }
            *param_4 = uVar1;
            *param_7 = uVar2;
...
/**
 * @name  lzma_stream_buffer_decode
 * @brief Decodes .xz format stream data from a buffer in a single call.
 * @confidence 90%
 * @classification io
 * @address 0x00111480
 */

/* Decodes .xz format stream data from a buffer. Validates input/output positions, initializes a BCJ
   filter, invokes the decoding pipeline, and handles error/cleanup conditions. Returns LZMA error
   codes (0=OK, 1=STREAM_END, 0xb=PROG_ERROR, etc.). */

int lzma_stream_buffer_decode
              (undefined8 *param_1,uint param_2,undefined8 param_3,long param_4,ulong *param_5,
              ulong param_6,long param_7,ulong *param_8,ulong param_9)

{
  ...
  if (param_5 != (ulong *)0x0) {
    if ((param_4 == 0) && (iVar3 = 0xb, param_6 != *param_5)) goto LAB_00111575;
    if ((param_8 != (ulong *)0x0) && (*param_5 <= param_6)) {
      iVar3 = 0xb;
      if (param_7 == 0) {
        if (param_9 != *param_8) goto LAB_00111575;
      }
      else if (param_9 < *param_8) goto LAB_00111575;
      iVar3 = 0xb;
      if ((param_2 & 4) == 0) {
        local_88 = (undefined1  [16])0x0;
        local_98 = ZEXT816(0xffffffffffffffff) << 0x40;
        ...
        local_68 = (undefined1  [16])0x0;
        local_58 = (undefined1  [16])0x0;
        iVar3 = lzma_stream_decoder_init(local_98,param_3,*param_1,param_2);
        if (iVar3 == 0) {
          uVar1 = *param_5;
          uVar2 = *param_8;
          iVar4 = (*(code *)local_88._8_8_)
                            (local_98._0_8_,param_3,param_4,param_5,param_6,param_7,param_8,param_9,
                             3);
          if (iVar4 != 1) {
            *param_5 = uVar1;
            *param_8 = uVar2;
            if (iVar4 == 0) {
              iVar3 = (*param_5 != param_6) + 9;
            }
            else {
              iVar3 = iVar4;
              if (iVar4 == 6) {
                (*(code *)local_68._8_8_)(local_98._0_8_,param_1,local_a0,0);
              }
            }
          }
        }
        lzma_next_coder_end(local_98,param_3);
      }
      goto LAB_00111575;
    }
  }
...
/* Allocates memory via custom allocator or standard malloc, minimum 1 byte */
 ...
 * @brief Allocates zeroed memory via custom allocator or calloc, minimum 1 byte
 * @confidence 95%
 * @classification memory
 * @address 0x00105110
 */

/* Allocates zeroed memory via allocator or calloc */

void * lzma_alloc_zero(size_t param_1,undefined8 *param_2)

{
  ...
  if ((param_2 != (undefined8 *)0x0) && ((code *)*param_2 != (code *)0x0)) {
    __s = (void *)(*(code *)*param_2)(param_2[2],1,__size);
    pvVar1 = (void *)0x0;
    if (__s != (void *)0x0) {
...
/* Frees memory using custom allocator's free function or standard free() */
...
/* Allocates and zero-initializes a 0x50-byte index stream node with default capacity 0x200 */
...
/* Pops front element from output queue, decrements count, adjusts memory, frees node */
...
/* Removes the head node from the output queue linked list, adjusts counters and memory accounting
    */

void lzma_outq_remove_head(long *param_1,undefined8 param_2)
 ...
 * @brief Drains output queue items until reaching a specific marker value in the third field
 * @confidence 80%
 * @classification memory
 * @address 0x00109ad0
 */

/* Drains output queue items until reaching a specific marker value */
...
/**
 * @name  lzma_outq_reset
 * @brief Resets output queue, drains items, resizes if needed, enforces max 0x4000 limit
 * @confidence 85%
 * @classification memory
 * @address 0x00109b30
 */

/* Resets output queue, drains items, resizes if needed */

undefined8 lzma_outq_reset(long *param_1,undefined8 param_2,uint param_3)

{
  ...
  while (param_3 * 2 < uVar1) {
    lzma_outq_pop(param_1,param_2);
    uVar1 = *(uint *)((long)param_1 + 0x34);
  }
  *(uint *)(param_1 + 7) = param_3 * 2;
  param_1[2] = 0;
  return 0;
}
...
/* Allocates output queue buffer of specified size. Reuses existing if same size, otherwise frees
   old buffers first */
...
/* Enqueues a new element into output queue linked list, initializes fields, updates counters and
   memory tracking */
...
/* Gets/sets index decoder memory configuration using index memusage calculation */
...
/* Queries/sets memory configuration for MT decoder. Sums buffer sizes, enforces minimum 0x8000 */

undefined8
lzma_stream_decoder_mt_memconfig(long param_1,ulong *param_2,undefined8 *param_3,ulong param_4)
 ...
 * @brief Conditionally allocates and copies delta filter properties. Returns 0 on success, 5 for OOM, 8 for invalid size
 * @confidence 75%
 * @classification memory
 * @address 0x00121d40
 */

/* Conditionally allocates and copies delta filter properties. Returns 0 on success, 5 for OOM, 8
   for invalid size */
...
/* Appends string to buffer at current position with 799-byte max constraint */
...
/* Formats a number with optional binary size units (KiB, MiB, GiB) for human-readable output. */

void lzma_str_format_number(undefined8 *param_1,uint param_2,char param_3)
 ...
 * @brief Converts LZMA filter chain array to human-readable string with configurable output format.
 * @confidence 88%
 * @classification string
 * @address 0x001080b0
 */

/* Converts LZMA filter chain array to human-readable string. Iterates through filter chain (max 4),
   looks up filter IDs in table, formats filter names and options with configurable output format
   flags. */
...
LAB_00108278:
    ...
    if (((param_3 & 0xffffff0f) == 0) && (*param_2 != -1)) {
      ...
      if (local_58 == 0) {
        ...
        local_50[0] = 0;
        local_90 = 0;
        puVar7 = &filter_str_separator_colon;
        if ((param_3 & 0x40) != 0) {
          puVar7 = &filter_str_separator_equals;
        }
        if (*param_2 == -1) {
LAB_001084a0:
          ...
          *param_1 = local_58;
          ...
          do {
            ...
            if (uVar6 != 0) {
              lzma_str_append(local_58,local_50,&filter_str_prefix_dash);
            }
            lVar14 = 0;
                ...
                if ((ulong)bVar2 != 0) {
                  ...
                  do {
                    if ((puVar11[0xc] != '\x03') &&
                      ...
                      lzma_str_append(local_58,local_50,&filter_str_separator_equals);
                      if ((puVar11[0xd] & 1) == 0) {
                        lzma_str_format_number(&local_58,iVar4,(byte)puVar11[0xd] >> 1 & 1);
                        puVar12 = &filter_str_separator_comma;
                      }
                      else {
 ...
 * @brief Generates formatted string listing available LZMA filters and their parameters.
 * @confidence 88%
 * @classification string
 * @address 0x001084c0
 */

/* Generates formatted string listing available LZMA filters and their parameters. Returns
   LZMA_OK(0), LZMA_PROG_ERROR(0xb), LZMA_MEM_ERROR(5), or LZMA_OPTIONS_ERROR(8). */
    ...
    *param_1 = 0;
    ...
    if ((param_3 & 0xffffff8e) == 0) {
      ...
      if (lVar3 != 0) {
        ...
        puVar5 = &filter_str_separator_colon;
        if ((param_3 & 0x40) != 0) {
          puVar5 = &filter_str_separator_equals;
        }
        bVar2 = false;
...
joined_r0x001085ac:
              ...
              if ((param_3 & 0x40) != 0) {
                lzma_str_append(lVar3,local_50,&filter_str_prefix_dash);
                lVar3 = local_58;
              }
              ...
              if ((param_3 & 0x30) != 0) {
                ...
                while( true ) {
                  ...
                  if (*(char *)(lVar3 + 0xc) == '\x03') {
                    ...
                    lzma_str_format_number(&local_58,*(undefined4 *)(lVar3 + 0x10),bVar8);
                    lzma_str_append(local_58,local_50,&DAT_001234f7);
                    lzma_str_format_number(&local_58,*(undefined4 *)(lVar3 + 0x14),bVar8);
                  }
                  else {
                  ...
                  puVar7 = &filter_str_separator_comma;
                }
              }
              bVar2 = true;
              lVar3 = local_58;
 ...
 * @brief Calculates bit cost of encoding a literal byte in LZMA using probability lookup tables.
 * @confidence 90%
 * @classification math
 * @address 0x0011ad00
 */

/* Calculates price (bit cost) of encoding a literal byte in LZMA using probability lookup tables.
    */

int lzma_literal_price(long param_1,int param_2,int param_3,char param_4,uint param_5,int param_6)

{
  ...
  if (param_4 == '\0') {
    ...
    do {
      uVar3 = uVar4 >> 1;
      iVar5 = iVar5 + (uint)(byte)(&probability_price_table)
...
/* XZ Utils backdoor initialization: parses ELF dynamic symbols from linker and hooks
   RSA_public_decrypt with a wrapper function */
...
/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */
/* CPU feature detection for CRC32: checks CPUID for CLMUL support, selects hardware-accelerated or
   software CRC32 implementation */

void lzma_crc32_init(undefined8 param_1,undefined8 param_2)

{
  ...
  if (*piVar1 != 0) {
    ...
    _DAT_001325c0 = lzma_crc32_clmul;
    if (uVar3 == 0x80202) goto LAB_00104e92;
  }
  _DAT_001325c0 = lzma_crc32_generic;
LAB_00104e92:
...
/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */
/* CPU feature detection for CRC64: checks CPUID for CLMUL support, selects hardware-accelerated or
   software CRC64 implementation */

void lzma_crc64_init(undefined8 param_1,undefined8 param_2)

{
  ...
  if (*piVar1 != 0) {
    ...
    iVar4 = *(int *)(lVar2 + 8);
    uVar3 = *(uint *)(lVar2 + 0xc) & 0x80202;
    _DAT_001325c8 = lzma_crc64_clmul;
    if (uVar3 == 0x80202) goto LAB_00104f02;
  }
  _DAT_001325c8 = lzma_crc64_generic;
LAB_00104f02:
  if (*(long *)(in_FS_OFFSET + 0x28) != *(long *)(in_FS_OFFSET + 0x28)) {
                    /* WARNING: Subroutine does not return */
    __stack_chk_fail(param_1,param_2,iVar4,uVar3);
...
/* Standard GCC runtime stub for deregistering TM clones */
...
/* Standard GCC runtime stub for registering TM clones */
...
/* Initializes internal LZMA stream state: allocates 0x68-byte structure, zeroes filter arrays and
   flags */

undefined8 lzma_strm_init(long param_1)
...
/* Initializes filter array with LZMA2 filter (ID 0x21) from preset level */
...
/**
 * @name  execute_with_mappings
 * @brief Processes filter array through a resolver callback to build a resolved mapping table, optionally reversed, then passes to filter chain init.
 * @confidence 70%
 * @classification init
 * @address 0x00105dd0
 */

/* Processes an array of mapping pairs through a resolver callback to build a resolved mapping table
   (with 3 fields per entry), optionally in reverse order, then passes it to set_filter_and_init.
   Likely part of LZMA filter chain initialization. */

ulong execute_with_mappings
                (undefined8 param_1,undefined8 param_2,long *param_3,code *param_4,char param_5)

{
  ...
  local_40 = *(long *)(in_FS_OFFSET + 0x28);
  if ((param_3 == (long *)0x0) || (*param_3 == -1)) {
    uVar2 = 0xb;
  }
  else {
    uVar2 = lzma_validate_filter_chain(param_3,&local_c0);
    if ((int)uVar2 == 0) {
      if (param_5 == '\0') {
        if (local_c0 != 0) {
          uVar2 = 0;
          ...
          do {
            ...
            if ((lVar3 == 0) || (*(long *)(lVar3 + 8) == 0)) goto LAB_00105e9a;
            plVar5[1] = *(long *)(lVar3 + 8);
            uVar2 = uVar2 + 1;
            lVar3 = *param_3;
            plVar5[2] = param_3[1];
            *plVar5 = lVar3;
        ...
        if (local_c0 != 0) {
          do {
            lVar4 = ~uVar2 + local_c0;
            lVar3 = (*param_4)(*param_3);
            if ((lVar3 == 0) || (*(long *)(lVar3 + 8) == 0)) goto LAB_00105e9a;
            lVar1 = *param_3;
            uVar2 = uVar2 + 1;
            local_b8[lVar4 * 3 + 1] = *(long *)(lVar3 + 8);
            lVar3 = param_3[1];
            local_b8[lVar4 * 3] = lVar1;
            local_b8[lVar4 * 3 + 2] = lVar3;
            param_3 = param_3 + 2;
          } while (uVar2 < local_c0);
        }
      }
      local_d0 = local_b8;
      local_b8[local_c0 * 3] = -1;
      local_b8[local_c0 * 3 + 1] = 0;
      uVar2 = lzma_next_filter_update(param_1,param_2,local_d0);
      if ((int)uVar2 != 0) {
        lzma_next_coder_end(param_1,param_2);
...
/* Allocates and initializes a 0xa8-byte stream node structure with provided parameters */

void lzma_index_stream_init
...
/* Allocates and initializes LZMA index structure with initial stream node */
...

{
  ...
  lVar3 = lVar1;
  if (lVar1 != 0) {
    lVar2 = lzma_index_stream_init(0,0,1,0,param_1);
    if (lVar2 == 0) {
      lVar3 = 0;
      lzma_free(lVar1,param_1);
    }
    else {
      lzma_index_tree_append(lVar1,lVar2);
    }
  }
  return lVar3;
}
...
/* Initializes from shared object: dlopen/dlinfo for path, or uses default linker r_debug, then
   parses ELF dynamic symbols */
...
/* Initializes LZ77 decoder: allocates 0x70-byte state, computes dictionary size (power of 2),
   delegates to match finder init. */
...

{
  ...
  if (pauVar3 == (undefined1 (*) [16])0x0) {
    ...
    *param_1 = pauVar3;
    param_1[3] = lzma_lz_decoder_code;
  ...
  if (cVar1 == '\0') {
    if (0xfff < *param_3) {
      ...
      *(uint *)(pauVar3[6] + 1) = uVar4 + (uVar4 != 0xffffffff);
      ...
      local_58 = param_3;
      uVar2 = lzma_next_filter_update(pauVar3,param_2,&local_68);
    }
  }
LAB_00109f8b:
  if (local_30 == *(long *)(in_FS_OFFSET + 0x28)) {
 ...
 * @brief Public API: Initializes LZMA alone format encoder
 * @confidence 90%
 * @classification init
 * @address 0x0010a110
 */

/* Public API: Initializes LZMA alone format encoder */

ulong lzma_alone_encoder(long param_1,undefined8 param_2)

{
  long lVar1;
  ulong uVar2;
  
  uVar2 = lzma_strm_init();
  if ((int)uVar2 == 0) {
    uVar2 = lzma_lz_decoder_init
...
/* Initializes block encoder: validates check type, allocates 0xe0-byte state, sets up
   encode/end/update callbacks. */

undefined8 lzma_block_encoder_init(undefined8 *param_1,undefined8 param_2,uint *param_3)

{
  ...
  if ((code *)param_1[2] != lzma_block_encoder_init) {
    lzma_next_coder_end();
  }
  param_1[2] = lzma_block_encoder_init;
  if (param_3 != (uint *)0x0) {
    if (1 < *param_3) {
      return 8;
    }
    ...
    if (uVar4 < 0x10) {
      ...
      if (pauVar3 == (undefined1 (*) [16])0x0) {
        ...
        *param_1 = pauVar3;
        ...
        param_1[4] = lzma_block_encoder_end;
 ...
 * @brief Public API: Initializes LZMA block encoder with bidirectional support flags
 * @confidence 90%
 * @classification init
 * @address 0x0010abb0
 */

/* Public API: Initializes LZMA block encoder */

ulong lzma_block_encoder(long param_1,undefined8 param_2)
 ...
 * @brief Public API: Initialize easy LZMA encoder with preset and check type
 * @confidence 92%
 * @classification init
 * @address 0x0010af80
 */

/* Public API: Initialize easy LZMA encoder with preset and check type */

undefined8 lzma_easy_encoder(undefined8 param_1,undefined8 param_2,undefined4 param_3)

{
  ...
  local_20 = *(long *)(in_FS_OFFSET + 0x28);
  cVar1 = lzma_easy_preset(auStack_e8);
  uVar2 = 8;
  if (cVar1 == '\0') {
    uVar2 = lzma_stream_encoder(param_1,auStack_e8,param_3);
  }
  if (local_20 == *(long *)(in_FS_OFFSET + 0x28)) {
    return uVar2;
  }
 ...
 * @brief Initializes next filter in chain via execute_with_mappings trampoline
 * @confidence 70%
 * @classification init
 * @address 0x0010b270
 */

/* Initializes next filter in chain via execute_with_mappings */
 ...
 * @brief Initializes raw LZMA encoder with filter configuration
 * @confidence 90%
 * @classification init
 * @address 0x0010b290
 */
...
/* Initializes index encoder internal coder with iterator, allocates 0x150-byte state */
 ...
 * @brief Public API: Initialize index encoder, sets allow_buf_error and supported_actions flags
 * @confidence 90%
 * @classification init
 * @address 0x0010baa0
 */

/* Public API: Initialize index encoder */

ulong lzma_index_encoder(long param_1,undefined8 param_2)

{
  ...
  uVar2 = lzma_strm_init();
  if ((int)uVar2 == 0) {
    uVar2 = lzma_index_encoder_init_coder
                      (*(undefined8 *)(param_1 + 0x38),*(undefined8 *)(param_1 + 0x30),param_2);
    if ((int)uVar2 == 0) {
      lVar1 = *(long *)(param_1 + 0x38);
      *(undefined1 *)(lVar1 + 0x60) = 1;
...
/**
 * @name  lzma_block_filters_setup
 * @brief Sets up block encoder/decoder filters. For initial states copies filters and initializes block. For active states delegates to update function.
 * @confidence 80%
 * @classification init
 * @address 0x0010c2a0
 */

/* Sets up block decoder filters. For state < 2 (initial setup), copies filters, computes block
   header size, initializes decoder. For states 2-3, delegates to a function pointer. Copies filter
   array into state on success. Returns liblzma error codes. */

int lzma_block_filters_setup(uint *param_1,undefined8 param_2,undefined8 param_3,undefined8 param_4)

{
  ...
  local_40 = *(long *)(in_FS_OFFSET + 0x28);
  iVar1 = lzma_filters_copy(param_3,&local_98,param_2);
  if (iVar1 != 0) goto LAB_0010c35a;
  if (*param_1 < 2) {
    *(undefined1 *)(param_1 + 1) = 0;
    *(undefined8 **)(param_1 + 0x1e) = &local_98;
    param_1[0x1a] = 0xffffffff;
    param_1[0x1b] = 0xffffffff;
    param_1[0x1c] = 0xffffffff;
    param_1[0x1d] = 0xffffffff;
    iVar2 = lzma_block_header_size(param_1 + 0x16);
    if (iVar2 == 0) {
      iVar2 = lzma_block_encoder_init(param_1 + 2,param_2,param_1 + 0x16);
      *(uint **)(param_1 + 0x1e) = param_1 + 0x4a;
      if (iVar2 == 0) {
    ...
    iVar2 = (**(code **)(param_1 + 0x12))(*(undefined8 *)(param_1 + 2),param_2,param_3,param_4);
    if (iVar2 == 0) {
LAB_0010c319:
      lzma_filters_free(param_1 + 0x4a,param_2);
      *(undefined8 *)(param_1 + 0x4a) = local_98;
      *(undefined8 *)(param_1 + 0x4c) = uStack_90;
      *(undefined8 *)(param_1 + 0x4e) = local_88;
      *(undefined8 *)(param_1 + 0x50) = uStack_80;
      *(undefined8 *)(param_1 + 0x52) = local_78;
      *(undefined8 *)(param_1 + 0x54) = uStack_70;
      *(undefined8 *)(param_1 + 0x56) = local_68;
      *(undefined8 *)(param_1 + 0x58) = uStack_60;
      *(undefined8 *)(param_1 + 0x5a) = local_58;
      *(undefined8 *)(param_1 + 0x5c) = uStack_50;
      goto LAB_0010c35a;
    }
  }
  else {
 ...
 * @brief Initializes LZMA stream encoder: allocates 0x5e0-byte state, creates index, encodes stream header, sets up block filters.
 * @confidence 88%
 * @classification init
 * @address 0x0010c410
 */

/* Initializes LZMA stream encoder: allocates 0x5e0-byte state, sets up encode/cleanup function
   pointers, creates index, encodes stream header, sets up block filters. */
  ...
  if (puVar3 == (undefined4 *)0x0) {
    ...
    if (puVar3 != (undefined4 *)0x0) {
      *param_1 = puVar3;
      ...
      param_1[4] = lzma_stream_decoder_mt_end;
...
/* Public API for initializing XZ stream encoder with filter chain and check type */

ulong lzma_stream_encoder(long param_1,undefined8 param_2,undefined4 param_3)
...
/* Configures LZMA encoder filters by validating memory usage, copying filter array, and freeing old
   filters. */
 ...
 * @brief Initializes multi-threaded LZMA stream encoder: mutexes, condvars (CLOCK_MONOTONIC), worker thread structures, filter chains, index, stream header.
 * @confidence 85%
 * @classification init
 * @address 0x0010d240
 */

/* Initializes multi-threaded LZMA stream encoder. Sets up mutexes, condition variables
   (CLOCK_MONOTONIC support), allocates worker thread structures, copies filter chains, creates
   index, encodes stream header. */

int lzma_stream_encoder_mt_init_coder(undefined8 *param_1,undefined8 param_2,long param_3)

{
  ...
  local_40 = *(long *)(in_FS_OFFSET + 0x28);
  if ((code *)param_1[2] != lzma_stream_encoder_mt_init_coder) {
    lzma_next_coder_end();
  }
  param_1[2] = lzma_stream_encoder_mt_init_coder;
...
LAB_0010d56c:
        ...
        param_1[4] = lzma_stream_decoder_mt_end;
        param_1[5] = lzma_stream_encoder_mt_get_progress;
...
LAB_0010d2ee:
    ...
    *(undefined8 *)(puVar6 + 0x66) = local_130;
    if (*(int *)(param_3 + 4) == iVar4) {
      lzma_mt_decoder_signal_workers(puVar6,1);
      uVar2 = *(undefined4 *)(param_3 + 4);
    }
    else {
      lzma_mt_stop_and_free_workers(puVar6 + 0x6a,puVar6 + 0x6d,param_2);
      *(undefined8 *)(puVar6 + 0x6c) = 0;
 ...
 * @brief Public API for initializing multi-threaded LZMA stream encoder
 * @confidence 90%
 * @classification init
 * @address 0x0010e230
 */

/* Public API for initializing multi-threaded LZMA stream encoder */
...

{
  ...
  uVar2 = lzma_strm_init();
  if ((int)uVar2 == 0) {
    uVar2 = lzma_stream_encoder_mt_init_coder
                      (*(undefined8 *)(param_1 + 0x38),*(undefined8 *)(param_1 + 0x30),param_2);
    if ((int)uVar2 == 0) {
      lVar1 = *(long *)(param_1 + 0x38);
      *(undefined1 *)(lVar1 + 0x60) = 1;
...
/* Initializes LZMA2 encoder filter: allocates 0x58-byte state, encodes lclppb, delegates to inner
   encoder init. */

undefined8
lzma_lzma2_encoder_init_wrapper(undefined8 *param_1,undefined8 param_2,undefined8 param_3)

{
  ...
  local_30 = *(long *)(in_FS_OFFSET + 0x28);
  if ((code *)param_1[2] != lzma_lzma2_encoder_init_wrapper) {
    lzma_next_coder_end();
  }
  param_1[2] = lzma_lzma2_encoder_init_wrapper;
  pauVar3 = (undefined1 (*) [16])*param_1;
  if (pauVar3 == (undefined1 (*) [16])0x0) {
    ...
    *param_1 = pauVar3;
    param_1[3] = lzma_simple_coder_decode;
...
/* Public API: Initializes MicroLZMA encoder, sets internal flag on success */
...

{
  ...
  uVar1 = lzma_strm_init();
  if ((int)uVar1 == 0) {
    uVar1 = lzma_lzma2_encoder_init_wrapper
                      (*(undefined8 *)(param_1 + 0x38),*(undefined8 *)(param_1 + 0x30),param_2);
    if ((int)uVar1 == 0) {
...
/* Initializes LZMA alone decoder coder with 0xe8-byte state, function pointers, and memory limit
   config. */

undefined8
lzma_alone_decoder_coder_init
          (undefined8 *param_1,undefined8 param_2,long param_3,undefined1 param_4)

{
  undefined1 (*pauVar1) [16];
  
  if ((code *)param_1[2] != lzma_alone_decoder_coder_init) {
    lzma_next_coder_end();
  }
  pauVar1 = (undefined1 (*) [16])*param_1;
  param_1[2] = lzma_alone_decoder_coder_init;
  if (pauVar1 == (undefined1 (*) [16])0x0) {
    ...
    *param_1 = pauVar1;
    param_1[3] = lzma_lzma2_decoder_code;
    param_1[4] = lzma_lzma2_decoder_end;
    param_1[7] = lzma_lzma2_decoder_memconfig;
...
/* Public API for .lzma format decoder initialization */
...

{
  ...
  uVar2 = lzma_strm_init();
  if ((int)uVar2 == 0) {
    uVar2 = lzma_alone_decoder_coder_init
                      (*(undefined8 *)(param_1 + 0x38),*(undefined8 *)(param_1 + 0x30),param_2,0);
    if ((int)uVar2 == 0) {
      lVar1 = *(long *)(param_1 + 0x38);
      *(undefined1 *)(lVar1 + 0x60) = 1;
      *(undefined1 *)(lVar1 + 99) = 1;
 ...
 * @brief Auto-detect decoder initialization
 * @confidence 88%
 * @classification init
 * @address 0x0010eb70
 */

/* Initializes auto-detect decoder: allocates 0x60-byte state, sets up decode/end/memconfig
   callbacks */

uint lzma_auto_decoder_init(undefined8 *param_1,undefined8 param_2,long param_3,uint param_4)

{
  undefined1 (*pauVar1) [16];
  
  if ((code *)param_1[2] != lzma_auto_decoder_init) {
    lzma_next_coder_end();
  }
  param_1[2] = lzma_auto_decoder_init;
  if ((param_4 & 0xffffffc0) == 0) {
    ...
    if (pauVar1 == (undefined1 (*) [16])0x0) {
      ...
      *param_1 = pauVar1;
      param_1[3] = lzma_auto_decoder_code;
      param_1[4] = lzma_block_decoder_end;
      param_1[6] = lzma_simple_coder_get_check;
      param_1[7] = lzma_stream_decoder_memconfig;
      *pauVar1 = (undefined1  [16])0x0;
      pauVar1[1] = (undefined1  [16])0x0;
      *(undefined8 *)(*pauVar1 + 8) = 0xffffffffffffffff;
      pauVar1[2] = (undefined1  [16])0x0;
 ...
 * @brief Initializes automatic format-detecting LZMA decoder
 * @confidence 90%
 * @classification init
 * @address 0x0010ee00
 */

/* Initializes automatic format-detecting LZMA decoder */
 ...
 * @brief Initializes LZMA block decoder: validates block parameters, allocates ~0xf8-byte state, sets up callbacks, initializes check handling.
 * @confidence 88%
 * @classification init
 * @address 0x0010f360
 */

/* Initializes an LZMA block decoder. Validates block parameters (unpadded size, uncompressed size),
   allocates decoder state (~0xf8 bytes), sets up decode/end callbacks, configures
   compressed/uncompressed size limits, initializes check handling, and delegates to filter chain
   initialization. */

undefined8 lzma_block_decoder_init(undefined8 *param_1,undefined8 param_2,int *param_3)

{
  ...
  if ((code *)param_1[2] != lzma_block_decoder_init) {
    lzma_next_coder_end();
  }
  param_1[2] = lzma_block_decoder_init;
  lVar3 = lzma_block_unpadded_size(param_3);
  if ((lVar3 != 0) &&
     (lVar3 = *(long *)(param_3 + 6), 0x7ffffffffffffffe < lVar3 + 0x8000000000000000U)) {
    puVar6 = (undefined4 *)*param_1;
    if (puVar6 == (undefined4 *)0x0) {
      puVar6 = (undefined4 *)lzma_alloc(0xf8,param_2);
      if (puVar6 == (undefined4 *)0x0) {
        return 5;
      }
      *param_1 = puVar6;
      lVar3 = *(long *)(param_3 + 6);
      param_1[3] = lzma_block_decoder_code;
      param_1[4] = lzma_simple_coder_end;
    ...
    *puVar6 = 0;
    *(int **)(puVar6 + 0x16) = param_3;
    iVar1 = param_3[2];
    *(undefined8 *)(puVar6 + 0x18) = 0;
    *(undefined8 *)(puVar6 + 0x1a) = 0;
    if (lVar5 == -1) {
      uVar2 = lzma_check_size(iVar1);
      lVar5 = (0x7ffffffffffffffc - (ulong)(uint)param_3[1]) - (ulong)uVar2;
    }
    *(long *)(puVar6 + 0x1c) = lVar5;
    if (lVar3 == -1) {
      lVar3 = 0x7fffffffffffffff;
    }
    *(undefined8 *)(puVar6 + 0x20) = 0;
    *(long *)(puVar6 + 0x1e) = lVar3;
    lzma_check_init(puVar6 + 0x22,iVar1);
    bVar7 = false;
    if (*param_3 != 0) {
      bVar7 = (char)param_3[0x32] != '\0';
    }
    *(bool *)(puVar6 + 0x3c) = bVar7;
    uVar4 = lzma_raw_decoder_init(puVar6 + 2,param_2,*(undefined8 *)(param_3 + 8));
    return uVar4;
  }
  return 0xb;
}
 ...
 * @brief Public API: Initializes LZMA block decoder
 * @confidence 90%
 * @classification init
 * @address 0x0010f510
 */

/* Public API: Initializes LZMA block decoder */

ulong lzma_block_decoder(long param_1,undefined8 param_2)
...
/* Initializes file info decoder: allocates 0x2160-byte state, sets up decode/end/memconfig
   callbacks, clears indices. */
...

{
  ...
  if ((code *)param_1[2] != lzma_file_info_decoder_init) {
    lzma_next_coder_end();
  }
  param_1[2] = lzma_file_info_decoder_init;
  if (param_4 != 0) {
    puVar1 = (undefined4 *)*param_1;
    if (puVar1 == (undefined4 *)0x0) {
      ...
      *param_1 = puVar1;
      ...
      param_1[7] = lzma_file_info_decoder_memconfig;
...
/* Initializes file info decoder from LZMA stream for seeking and index extraction */
...

{
  ...
  uVar2 = lzma_strm_init();
  if ((int)uVar2 == 0) {
    uVar2 = lzma_file_info_decoder_init
 ...
 * @brief Initializes raw decoder filter chain via execute_with_mappings
 * @confidence 75%
 * @classification init
 * @address 0x00110600
 */

/* Initializes raw decoder filter chain via execute_with_mappings */

void lzma_raw_decoder_init(void)
 ...
 * @brief Initializes raw LZMA decoder with filter chain
 * @confidence 90%
 * @classification init
 * @address 0x00110620
 */

/* Initializes raw LZMA decoder with filter chain */
...
/**
 * @name  lzma_index_decoder_new
 * @brief Creates and initializes a new LZMA index decoder. Allocates 0x48 bytes for decoder state
 * @confidence 85%
 * @classification init
 * @address 0x00110b70
 */

/* Creates and initializes a new LZMA index decoder. Allocates 0x48 bytes for decoder state */

undefined8
lzma_index_decoder_new(undefined8 *param_1,undefined8 param_2,undefined8 *param_3,long param_4)

{
  long lVar1;
  undefined4 *puVar2;
  
  if ((code *)param_1[2] != lzma_index_decoder_new) {
    lzma_next_coder_end();
  }
  param_1[2] = lzma_index_decoder_new;
  if (param_3 == (undefined8 *)0x0) {
    return 0xb;
  }
  puVar2 = (undefined4 *)*param_1;
  if (puVar2 == (undefined4 *)0x0) {
    puVar2 = (undefined4 *)lzma_alloc(0x48,param_2);
    if (puVar2 == (undefined4 *)0x0) {
      return 5;
    }
    *param_1 = puVar2;
    param_1[3] = lzma_index_decoder_run;
    param_1[4] = lzma_index_decoder_end;
    param_1[7] = lzma_index_decoder_memconfig;
    *(undefined8 *)(puVar2 + 4) = 0;
  }
  else {
    lzma_index_end(*(undefined8 *)(puVar2 + 4),param_2);
  }
...
/* Public API to initialize LZMA index decoder. Zeroes output index pointer, initializes decoder
   state */
...
/* Initializes LZMA index hash structure (0x140 bytes), zeroes field ranges, initializes SHA-256
   checks */
...
/* Initializes .xz stream decoder: allocates 0x588-byte state, extracts flag bits, initializes index
   hash, sets state to 0. */
 ...
 * @brief Public API to initialize LZMA stream decoder for .xz files
 * @confidence 92%
 * @classification init
 * @address 0x00111df0
 */

/* Public API to initialize LZMA stream decoder for .xz files */
 ...
 * @brief Initializes multi-threaded LZMA stream decoder: allocates 0x6e8-byte coder, pthread mutex/condvar, sets function pointers.
 * @confidence 88%
 * @classification init
 * @address 0x00113bf0
 */

/* Initializes multi-threaded LZMA stream decoder. Allocates 0x6e8-byte coder, initializes pthread
   mutex/condvar (monotonic clock if available), sets function pointers, copies thread options,
   initializes filter chain and index hash. */

int lzma_stream_decoder_mt_init(undefined8 *param_1,undefined8 param_2,uint *param_3)

{
  ...
  local_40 = *(long *)(in_FS_OFFSET + 0x28);
  if ((code *)param_1[2] != lzma_stream_decoder_mt) {
    lzma_next_coder_end();
  }
  param_1[2] = lzma_stream_decoder_mt;
...
LAB_00113e3d:
        ...
        param_1[7] = lzma_stream_decoder_mt_memconfig;
        param_1[5] = lzma_stream_decoder_mt_get_progress;
...
LAB_00113c41:
    ...
    lzma_mt_decoder_stop_workers(puVar5,param_2);
 ...
 * @brief Validates MT decoder params then initializes multi-threaded stream decoder
 * @confidence 80%
 * @classification init
 * @address 0x00113fb0
 */

/* Validates MT decoder params (threads < 0x4001, flags) then initializes */

undefined8 lzma_stream_decoder_mt(undefined8 param_1,undefined8 param_2,uint *param_3)

{
  undefined8 uVar1;
  
  if ((param_3[1] - 1 < 0x4000) && ((*param_3 & 0xffffffc0) == 0)) {
    uVar1 = lzma_stream_decoder_mt_init();
    return uVar1;
  }
  return 8;
}
...
/* Initializes multi-threaded LZMA stream decoder with thread count and flag validation */
...

{
  ...
  iVar2 = lzma_strm_init();
  if (iVar2 != 0) {
    return iVar2;
  }
  if ((param_2[1] - 1 < 0x4000) && ((*param_2 & 0xffffffc0) == 0)) {
    iVar2 = lzma_stream_decoder_mt_init
                      (*(undefined8 *)(param_1 + 0x38),*(undefined8 *)(param_1 + 0x30),param_2);
    if (iVar2 == 0) {
      lVar1 = *(long *)(param_1 + 0x38);
      *(undefined1 *)(lVar1 + 0x60) = 1;
 ...
 * @brief Initializes encoder by allocating 0x68-byte state structure and setting up function pointers for microlzma decoder
 * @confidence 70%
 * @classification init
 * @address 0x00114060
 */

/* Initializes LZMA index encoder by allocating 0x68-byte state structure and setting up function
   pointers */

undefined8
lzma_index_encoder_init
          (undefined8 *param_1,undefined8 param_2,undefined8 param_3,long param_4,undefined1 param_5
          ,undefined4 param_6)

{
  undefined1 (*pauVar1) [16];
  
  if ((code *)param_1[2] != lzma_index_encoder_init) {
    lzma_next_coder_end();
  }
  param_1[2] = lzma_index_encoder_init;
  pauVar1 = (undefined1 (*) [16])*param_1;
  if (pauVar1 == (undefined1 (*) [16])0x0) {
    pauVar1 = (undefined1 (*) [16])lzma_alloc(0x68,param_2);
    if (pauVar1 == (undefined1 (*) [16])0x0) {
      return 5;
    }
    *param_1 = pauVar1;
    param_1[3] = lzma_microlzma_decoder_code;
    param_1[4] = lzma_delta_coder_end;
    *pauVar1 = (undefined1  [16])0x0;
    pauVar1[1] = (undefined1  [16])0x0;
    *(undefined8 *)(*pauVar1 + 8) = 0xffffffffffffffff;
    pauVar1[2] = (undefined1  [16])0x0;
  ...
  if (-1 < param_4) {
    *(undefined8 *)pauVar1[5] = param_3;
    *(long *)(pauVar1[5] + 8) = param_4;
    pauVar1[6][4] = param_5;
    *(undefined4 *)pauVar1[6] = param_6;
    pauVar1[6][5] = 0;
    return 0;
  }
  return 8;
 ...
 * @brief Initializes MicroLZMA decoder
 * @confidence 85%
 * @classification init
 * @address 0x00114420
 */

/* Initializes MicroLZMA decoder */
...
/* Initializes lzip format decoder: allocates 0x118-byte state, sets up decoder/end callbacks,
   extracts flag bits. */

uint lzma_lzip_decoder_init(undefined8 *param_1,undefined8 param_2,long param_3,ulong param_4)

{
  undefined4 *puVar1;
  
  if ((code *)param_1[2] != lzma_lzip_decoder_init) {
    lzma_next_coder_end();
  }
  param_1[2] = lzma_lzip_decoder_init;
  if ((param_4 & 0xffffffc0) == 0) {
    puVar1 = (undefined4 *)*param_1;
    if (puVar1 == (undefined4 *)0x0) {
      ...
      *param_1 = puVar1;
      param_1[3] = lzma_lzip_decoder_code;
      param_1[4] = lzma_lzip_decoder_end;
      param_1[6] = FUN_001144b0;
      param_1[7] = lzma_lzip_decoder_memconfig;
...
/**
 * @name  lzma_lzip_decoder
 * @brief Public API: Initialize LZIP decoder
 * @confidence 88%
 * @classification init
 * @address 0x00114a50
 */

/* Public API: Initialize LZIP decoder */

ulong lzma_lzip_decoder(long param_1,undefined8 param_2,undefined4 param_3)
...
/* Initializes integrity check state: CRC32=0, CRC64=0, SHA256=init */
 ...
 * @brief Initializes LZ match finder: selects strategy via function pointers, computes hash/son buffer sizes, sets depth, reallocates when needed.
 * @confidence 85%
 * @classification init
 * @address 0x001168e0
 */

/* Initializes LZ match finder state. Selects strategy (hash chain/binary tree) via function
   pointers, computes hash/son buffer sizes from dictionary size, sets depth parameter, reallocates
   buffers when sizes change. */

undefined8 lzma_lz_encoder_mf_init(long *param_1,undefined8 param_2,long *param_3)

{
  ...
  *(int *)((long)param_1 + 0x54) = iVar12;
  ...
  case 3:
    ...
    param_1[7] = (long)lzma_mf_hc3_skip;
    break;
  case 4:
    param_1[6] = (long)lzma_mf_hc4_find;
    param_1[7] = (long)lzma_mf_bt4_skip;
    break;
  default:
    return 1;
  case 0x12:
    ...
    *(undefined4 *)((long)param_1 + 0x6c) = 0x10000;
    ...
    param_1[6] = (long)lzma_mf_hc3_find;
    param_1[7] = (long)lzma_mf_bt3_find;
    iVar13 = 0x10000;
    goto LAB_001169ed;
  case 0x13:
    uVar14 = 0x10;
    param_1[6] = (long)lzma_mf_hc3_find;
    param_1[7] = (long)lzma_mf_hc4_skip;
...
LAB_00116be0:
    *(uint *)(param_1 + 0xb) = uVar8;
    ...
    goto LAB_00116b3d;
  case 0x14:
    param_1[6] = (long)lzma_mf_bt4_find;
    param_1[7] = (long)lzma_mf_bt4_find_and_skip;
    uVar8 = iVar7 - 1U | iVar7 - 1U >> 1;
    uVar8 = uVar8 | uVar8 >> 2;
    uVar8 = uVar8 | uVar8 >> 4;
    uVar8 = (uVar8 | uVar8 >> 8) >> 1 | 0xffff;
...
/**
 * @name  lzma_lzma_decoder_init_v2
 * @brief Initializes LZMA decoder context (alternate version): allocates 0xf0-byte state, initializes probability tables, dictionary, preset dictionary.
 * @confidence 78%
 * @classification init
 * @address 0x00116ce0
 */

/* Initializes LZMA decoder context: allocates 0xf0-byte state, sets function pointers for stream
   decoding, initializes probability tables and dictionary, copies preset dictionary, chains to
   sub-filter. */

undefined8
lzma_lzma_decoder_init(undefined8 *param_1,undefined8 param_2,undefined8 *param_3,code *param_4)
...
LAB_00116d15:
    ...
    cVar2 = lzma_lz_encoder_mf_init(puVar4 + 5,param_2,auStack_78);
    uVar3 = 8;
    ...
    *(undefined4 *)(puVar4 + 0x12) = 0;
    uVar3 = lzma_next_filter_update(puVar4 + 0x14,param_2,param_3 + 3);
  }
LAB_00116d30:
  if (local_30 == *(long *)(in_FS_OFFSET + 0x28)) {
    return uVar3;
 ...
 * @brief Initializes LZMA decoder: allocates 0x10c0-byte coder, sets function pointers, allocates dictionary buffer with 0x240 header, copies preset dictionary.
 * @confidence 85%
 * @classification init
 * @address 0x00118570
 */

/* Initializes LZMA decoder: allocates 0x10c0-byte coder structure, sets function pointers for
   decode/cleanup, calls specific LZ init, allocates dictionary buffer (16-byte aligned with 0x240
   header), copies preset dictionary, chains to sub-filter init. */

undefined8
lzma_lzma_decoder_init(long *param_1,undefined8 param_2,undefined8 *param_3,code *param_4)

{
  ...
  if (plVar4 == (long *)0x0) {
    ...
    if (plVar4 != (long *)0x0) {
      *param_1 = (long)plVar4;
      param_1[3] = (long)lzma_lzma2_decoder_code;
...
LAB_001185a8:
    ...
    *(undefined2 *)(plVar4 + 0x15) = 0;
    plVar4[0x16] = 0;
    plVar4[0x17] = 0;
    uVar2 = lzma_next_filter_update(plVar4 + 0xb,param_2,param_3 + 3);
  }
LAB_00118684:
  if (local_30 == *(long *)(in_FS_OFFSET + 0x28)) {
    return uVar2;
...
/* Initializes LZMA compression options from preset level (0-9). Configures dict size, match finder,
   nice length, depth */
 ...
 * @brief Initializes LZMA literal encoding price tables. Computes cumulative bit costs from probability tables for three contexts.
 * @confidence 82%
 * @classification init
 * @address 0x001188c0
 */

/* Initializes LZMA literal encoding price tables. Computes cumulative bit costs from probability
   tables using a lookup table that maps probabilities to approximate bit costs for three literal
   coding contexts. */
...

{
  ...
  *(uint *)(param_1 + param_2 * 2 + 0x2404) = uVar6;
  bVar1 = (&probability_price_table)[uVar5 >> 4];
  bVar2 = (&probability_price_table)[uVar5 >> 4 ^ 0x7f];
  puVar9 = param_1 + param_2 * 0x220 + 0x202;
  bVar3 = (&probability_price_table)[param_1[1] >> 4];
  bVar4 = (&probability_price_table)[param_1[1] >> 4 ^ 0x7f];
  if (uVar6 != 0) {
    uVar11 = 0;
    puVar13 = puVar9;
    do {
      ...
      do {
        uVar7 = uVar12 >> 1;
        iVar10 = iVar10 + (uint)(byte)(&probability_price_table)
 ...
 * @brief Initializes LZMA encoder probability tables with 0x400 default values across multiple offset ranges.
 * @confidence 85%
 * @classification init
 * @address 0x00118b20
 */

/* Initializes LZMA encoder probability tables with 0x400 values across multiple offset ranges. */

void lzma_lzma_encoder_reset_prices(undefined4 *param_1,uint param_2,char param_3)
 ...
 * @brief Resets LZMA encoder: validates properties, initializes position state mask, probability tables to 0x400, length encoder tables.
 * @confidence 85%
 * @classification init
 * @address 0x0011a340
 */

/* Resets LZMA encoder internal state. Validates filter properties, initializes position state mask,
   literal context/position bits, all range encoder probability tables to 0x400, and length encoder
   tables. */

undefined8 lzma_lzma_encoder_reset(undefined8 *param_1,long param_2)

{
  ...
  if (cVar5 != '\0') {
    ...
    *(undefined4 *)(param_1 + 0x5b) = 0;
    ...
    lzma_lzma_encoder_reset_prices(param_1 + 0xe3d,iVar17,*(undefined1 *)((long)param_1 + 0xb74));
    lzma_lzma_encoder_reset_prices
 ...
 * @brief Initializes LZMA/LZMA2 encoder: allocates ~0x3CED0-byte state, configures dictionary/match finder parameters.
 * @confidence 82%
 * @classification init
 * @address 0x0011a570
 */

/* Initializes an LZMA/LZMA2 encoder. Allocates a large state structure (~0x3CED0 bytes), configures
   encoder parameters (dictionary size, literal context/position bits, match finder mode), populates
   output filter descriptor, and delegates to inner encoder initialization. Supports two modes
   (1=fast, 2=normal) with dictionary size power-of-2 calculation. */
  ...
  if (param_4[8] == 1) {
    ...
    *(undefined1 *)(lVar5 + 0xb74) = 0;
    ...
    if (1 < uVar1) {
      do {
        iVar6 = iVar6 + 1;
      } while ((uint)(1 << ((byte)iVar6 & 0x1f)) < uVar1);
      iVar6 = iVar6 * 2;
    }
    uVar1 = param_4[10];
    uVar2 = param_4[9];
    *(int *)(lVar5 + 0x10e78) = iVar6;
    uVar7 = uVar1 & 0xf;
    if ((uVar1 & 0xf) < uVar2) {
      uVar7 = uVar2;
    }
    *(uint *)(lVar5 + 0xb9ec) = uVar7 - 1;
    *(uint *)(lVar5 + 0x10234) = uVar7 - 1;
  }
  lVar3 = *(long *)(param_4 + 2);
  bVar8 = false;
  if (lVar3 != 0) {
    bVar8 = param_4[4] != 0;
  }
  *(bool *)(lVar5 + 0xb75) = bVar8;
  *(undefined1 *)(lVar5 + 0xb76) = 0;
  *(bool *)(lVar5 + 0xb77) = param_3 == 0x4000000000000001;
  *(undefined8 *)(lVar5 + 0x2b0) = 0;
  *(undefined8 *)(lVar5 + 0x2c0) = 0;
  *(undefined8 *)(lVar5 + 0x2b8) = 0;
  if (param_3 == 0x4000000000000002) {
    if ((param_4[0xc] & 0xfffffffe) != 0) {
      return 8;
    }
    *(char *)(lVar5 + 0xb77) = (char)param_4[0xc];
    *(byte *)(lVar5 + 0xb77) = *(byte *)(lVar5 + 0xb77) & 1;
  }
  uVar1 = *param_4;
  uVar2 = param_4[10];
  *param_5 = 0x1000;
  param_5[2] = 0x1001;
  param_5[1] = (ulong)uVar1;
  uVar1 = param_4[9];
  uVar7 = uVar2 & 0xf;
  param_5[3] = 0x111;
  *(uint *)(param_5 + 5) = uVar2;
  if (uVar1 < uVar7) {
    uVar1 = uVar7;
  }
  param_5[4] = (ulong)uVar1;
  uVar1 = param_4[0xb];
  param_5[6] = lVar3;
  *(uint *)((long)param_5 + 0x2c) = uVar1;
  *(uint *)(param_5 + 7) = param_4[4];
  uVar4 = lzma_lzma_encoder_reset(lVar5,param_4);
  return uVar4;
}
...
/* Sets LZMA2 encoder function pointers for encode and set_out_limit, initializes sub-encoder */
 ...
 * @brief Resets all LZMA range decoder probability tables to default 0x400. Initializes literal, match, rep, len, position slot, alignment coders.
 * @confidence 88%
 * @classification init
 * @address 0x0011ccd0
 */

/* Resets all LZMA range decoder probability tables to default mid-probability (0x400). Initializes
   literal coders, match/rep/len coders, position slot coders, alignment bits, and state variables
   based on lc/lp/pb properties. */

void lzma_lzma_decoder_reset(undefined2 *param_1,long param_2)
...
/**
 * @name  lzma_lzma_decoder_init_state
 * @brief Allocates 0x6ec0-byte LZMA decoder state structure, sets function pointers, copies properties
 * @confidence 80%
 * @classification init
 * @address 0x00120340
 */

/* Allocates 0x6ec0-byte LZMA decoder state structure and copies properties */

undefined8
lzma_lzma_decoder_init_state(long *param_1,undefined8 param_2,uint *param_3,ulong *param_4)

{
  long lVar1;
  
  if (*param_1 == 0) {
    lVar1 = lzma_alloc(0x6ec0);
    *param_1 = lVar1;
    if (lVar1 == 0) {
      return 5;
    }
    param_1[1] = (long)lzma_lzma_decode;
    param_1[2] = (long)lzma_lzma_decoder_reset;
    param_1[3] = (long)lzma_lzma_encoder_set_limits;
  }
...
/* Creates/initializes LZMA decoder state with lclppb validation, range decoder init, and
   uncompressed size configuration */

int lzma_lzma_decoder_create
...

{
  ...
  if ((((*(uint *)(param_4 + 0x14) < 5) && (*(uint *)(param_4 + 0x18) < 5)) &&
    ...
    if (iVar2 == 0) {
      lzma_lzma_decoder_reset(*param_1,param_4);
      lVar1 = *param_1;
...
/* Initializes LZMA encoder: allocates 0x100a8-byte state, copies options, sets up callbacks,
   ensures min dict size 0x10000. */
...

{
  ...
  if (puVar3 == (undefined4 *)0x0) {
    ...
    *param_1 = puVar3;
    param_1[1] = lzma_lzma2_encoder_encode;
...
/* Initializes LZMA1 decoder: allocates 0xb8-byte state, sets up decode/end callbacks, calls filter
   init */
...

{
  ...
  if (puVar3 == (undefined4 *)0x0) {
    ...
    *param_1 = puVar3;
    param_1[1] = lzma_lzma2_decoder_decode;
...
/* Creates and initializes a delta codec context (0x70 bytes) from a property byte encoding distance
   parameters */

undefined8 lzma_delta_coder_init(undefined8 *param_1,undefined8 param_2,byte *param_3,long param_4)
...
/**
 * @name  branch_filter_init
 * @brief Initializes a BCJ branch filter decoder. Allocates 0x160-byte state, validates options, sets up internal buffer.
 * @confidence 80%
 * @classification init
 * @address 0x00121360
 */

/* Initializes a branch prediction/BCJ filter decoder. Allocates filter state if needed, validates
   options from a filters array, and sets up internal state with size limits. */

undefined8 branch_filter_init(undefined8 *param_1,undefined8 param_2,long param_3)

{
  ...
  if (pauVar4 == (undefined1 (*) [16])0x0) {
    pauVar4 = (undefined1 (*) [16])lzma_alloc(0x160);
    if (pauVar4 == (undefined1 (*) [16])0x0) {
      return 5;
    }
    *param_1 = pauVar4;
    param_1[4] = lzma_index_encoder_end;
    *pauVar4 = (undefined1  [16])0x0;
    pauVar4[1] = (undefined1  [16])0x0;
    *(undefined8 *)(*pauVar4 + 8) = 0xffffffffffffffff;
  ...
  if ((piVar2 != (int *)0x0) && (*piVar2 == 0)) {
    ...
    pauVar4[5][8] = 0;
    *(ulong *)pauVar4[5] = (ulong)uVar1;
    puVar6 = (undefined8 *)((ulong)(pauVar4[6] + 1) & 0xfffffffffffffff8);
    *(undefined8 *)(pauVar4[5] + 9) = 0;
    *(undefined8 *)(pauVar4[0x15] + 1) = 0;
    uVar5 = (ulong)(((int)pauVar4 - (int)puVar6) + 0x159U >> 3);
    for (; uVar5 != 0; uVar5 = uVar5 - 1) {
      *puVar6 = 0;
      puVar6 = puVar6 + (ulong)bVar7 * -2 + 1;
    }
    uVar3 = lzma_next_filter_update(pauVar4,param_2,param_3 + 0x18);
    return uVar3;
  }
  return 8;
}
...
/**
 * @name  lzma_bcj_decoder_init
 * @brief Sets up BCJ decoder function pointers for code and get_check, then calls branch_filter_init
 * @confidence 75%
 * @classification init
 * @address 0x00121600
 */

/* Sets up BCJ decoder function pointers for code and get_check, then calls branch_filter_init */

void lzma_bcj_decoder_init(long param_1)

{
  *(code **)(param_1 + 0x18) = lzma_delta_decoder_code;
...
/* Sets encoder code function pointer and calls branch_filter_init for BCJ filter */

void lzma_bcj_encoder_init(long param_1)

{
  *(code **)(param_1 + 0x18) = lzma_delta_encoder_code;
  branch_filter_init();
  return;
}
...
/* Initializes LZ encoder: allocates context with buffer, sets up encode/end callbacks, delegates to
   filter init. */
...

{
  ...
  if (pauVar3 == (undefined1 (*) [16])0x0) {
    ...
    *param_1 = pauVar3;
    param_1[3] = lzma_simple_coder_decode_with_buffering;
  ...
  *(undefined8 *)(pauVar3[8] + 8) = 0;
  uVar2 = lzma_next_filter_update(pauVar3,param_2,param_3 + 0x18);
  return uVar2;
}
...
/* Calls lzma_lz_encoder_init and on success initializes encoder state */
...
/**
 * @name  lzma_lzma1_encoder_init
 * @brief LZMA1 filter encoder initialization via lzma_lz_encoder_init
 * @confidence 65%
 * @classification init
 * @address 0x001220e0
 */

/* Tail-call wrapper for lzma_lz_encoder_init, likely LZMA1 filter encoder init */

void lzma_lzma1_encoder_init(void)
...
/**
 * @name  lzma_lzma2_encoder_init_wrapper
 * @brief LZMA2 filter encoder initialization via lzma_lz_encoder_init
 * @confidence 65%
 * @classification init
 * @address 0x00122110
 */

/* Tail-call wrapper for lzma_lz_encoder_init, specific filter encoder init */

void lzma_lzma2_encoder_init_wrapper(void)
 ...
 * @brief Tail-call wrapper for lzma_lz_encoder_init, likely a specific filter encoder init
 * @confidence 30%
 * @classification init
 * @address 0x00122310
 */

/* Tail-call wrapper for lzma_lz_encoder_init */
 ...
 * @brief Tail-call wrapper for lzma_lz_encoder_init, likely BCJ variant
 * @confidence 30%
 * @classification init
 * @address 0x00122340
 */

/* Tail-call wrapper for lzma_lz_encoder_init, likely a specific filter encoder init (e.g., BCJ
   variant) */
...
/* Standard CRT finalization: calls __cxa_finalize and cleanup functions */
 ...
 * @brief Calls destructor or frees coder data, then zeros out 5 fields of next_coder structure
 * @confidence 90%
 * @classification cleanup
 * @address 0x001051a0
 */

/* Calls destructor or free, then zeros out 5 fields of next_coder structure */
 ...
 * @brief Ends next coder if its end function pointer at offset 0x10 is non-null
 * @confidence 85%
 * @classification cleanup
 * @address 0x00105300
 */

/* Ends next coder if its end function pointer at offset 0x10 is non-null */
...
/* Main LZMA stream destructor: cleans up internal state and frees allocations */
...
/* Frees LZMA filter chain (up to 4 filters), clearing options and IDs */
...
/**
 * @name  lzma_index_tree_free
 * @brief Recursively traverses binary tree (left/right children at +0x18/+0x20) and invokes callback on each node
 * @confidence 85%
 * @classification cleanup
 * @address 0x00106090
 */

/* Recursively traverses binary tree (left/right children at +0x18/+0x20) and invokes callback on
   each node */

void lzma_index_tree_free(long param_1,undefined8 param_2,code *UNRECOVERED_JUMPTABLE)

{
  if (*(long *)(param_1 + 0x18) != 0) {
    lzma_index_tree_free();
  }
  if (*(long *)(param_1 + 0x20) != 0) {
    lzma_index_tree_free(*(long *)(param_1 + 0x20),param_2,UNRECOVERED_JUMPTABLE);
  }
                    /* WARNING: Could not recover jumptable at 0x001060ce. Too many branches */
                    /* WARNING: Treating indirect jump as call */
...
/* Frees tree at stream+0x38 if non-null, then frees stream node */
...
/* Destructor for LZMA index: recursively frees tree then deallocates index */
...
/* Repeatedly pops items from output queue until empty */
...
/* Fully drains output queue: removes active nodes then pops pending items */
 ...
 * @brief Ends next coder then frees LZ decoder coder allocation
 * @confidence 85%
 * @classification cleanup
 * @address 0x00109e60
 */

/* Ends next coder then frees the LZ decoder coder allocation */
...
/**
 * @name  lzma_block_encoder_end
 * @brief Ends next coder then frees block encoder coder allocation
 * @confidence 85%
 * @classification cleanup
 * @address 0x0010a800
 */

/* Ends next coder then frees the block encoder coder allocation */

void lzma_block_encoder_end(undefined8 param_1,undefined8 param_2)
...
/* Full MT stream decoder cleanup: two next coders, index, filters, coder */

void lzma_stream_decoder_mt_end(long param_1,undefined8 param_2)
...
/* Signals worker threads to state 4, joins all threads, frees worker array memory */

void lzma_mt_stop_and_free_workers(long *param_1,uint *param_2,undefined8 param_3)
...
/* Full cleanup for MT stream decoder: stops threads, drains queues, frees filters/index/sync
   objects */

void lzma_stream_decoder_mt_end(long param_1,undefined8 param_2)

{
  lzma_mt_stop_and_free_workers(param_1 + 0x1a8,param_1 + 0x1b4,param_2);
 ...
 * @brief Generic cleanup: ends next coder then frees coder allocation
 * @confidence 80%
 * @classification cleanup
 * @address 0x0010e5c0
 */

/* Generic cleanup: ends next coder then frees coder allocation */
...
/**
 * @name  lzma_lzma2_decoder_end
 * @brief Ends next coder then frees LZMA2 decoder coder allocation
 * @confidence 85%
 * @classification cleanup
 * @address 0x0010e790
 */

/* Ends next coder then frees LZMA2 decoder coder allocation */

void lzma_lzma2_decoder_end(undefined8 param_1,undefined8 param_2)
...
/**
 * @name  lzma_block_decoder_end
 * @brief Ends next coder then frees block decoder coder allocation
 * @confidence 85%
 * @classification cleanup
 * @address 0x0010ec70
 */

/* Ends next coder then frees block decoder coder allocation */

void lzma_block_decoder_end(undefined8 param_1,undefined8 param_2)
 ...
 * @brief Ends sub-coder at coder+8 then frees simple coder
 * @confidence 85%
 * @classification cleanup
 * @address 0x0010f030
 */

/* Ends sub-coder at coder+8 then frees simple coder */
...
/* Cleans up file info decoder: next coder, two index objects, and coder struct */
...
/* Frees index object at coder+0x10 then frees coder */
...
/* Cleans up stream decoder including next coder, index hash, and coder allocation */
...
/* Stops MT decoder worker threads: signals state 3, joins all threads, frees worker array, resets
   counters */

void lzma_mt_decoder_stop_workers(long param_1,undefined8 param_2)
 ...
 * @brief Cleans up MT stream encoder: stops threads, frees filters, index hash, etc.
 * @confidence 85%
 * @classification cleanup
 * @address 0x001126b0
 */

/* Cleans up MT stream encoder: stops threads, frees filters, index hash, etc. */

void lzma_stream_encoder_mt_end(long param_1,undefined8 param_2)

{
  lzma_mt_decoder_stop_workers();
...
/**
 * @name  lzma_delta_coder_end
 * @brief Ends next coder then frees delta coder allocation
 * @confidence 85%
 * @classification cleanup
 * @address 0x00114150
 */

/* Ends next coder then frees the delta coder allocation */

void lzma_delta_coder_end(undefined8 param_1,undefined8 param_2)
 ...
 * @brief Ends sub-coder at coder+200 then frees lzip decoder
 * @confidence 85%
 * @classification cleanup
 * @address 0x00114630
 */

/* Ends sub-coder at coder+200 then frees lzip decoder */
...
/* Frees LZMA encoder resources including sub-coders, buffers, and dictionary */
...
/* Frees LZMA decoder: ends sub-coder, frees dictionary (via destructor or free), frees coder */
 ...
 * @brief Frees nested decoder at coder+8 then frees coder struct
 * @confidence 85%
 * @classification cleanup
 * @address 0x00120830
 */

/* Frees nested decoder at coder+8 then frees coder struct */
...
/**
 * @name  lzma_index_encoder_end
 * @brief Ends next coder then frees index encoder coder allocation
 * @confidence 85%
 * @classification cleanup
 * @address 0x00121330
 */

/* Ends next coder then frees index encoder coder allocation */

void lzma_index_encoder_end(undefined8 param_1,undefined8 param_2)
...
/* Frees LZ encoder sub-resources at coder+0x60 and the coder itself */
...
/* Sets filter function pointers in codec state and calls the filter's initialization function */

undefined8 lzma_next_filter_update(long param_1,undefined8 param_2,undefined8 *param_3)
 ...
 * @brief Validates action matches expected, dispatches to coder function pointer via jump table
 * @confidence 75%
 * @classification handler
 * @address 0x001052c0
 */

/* Validates action matches expected, dispatches to coder function pointer */
...
/**
 * @name  lzma_code
 * @brief Main LZMA codec dispatch function. Validates stream state, checks action validity, calls internal codec, updates positions/byte counts.
 * @confidence 95%
 * @classification handler
 * @address 0x001053c0
 */

/* Main LZMA codec dispatch function. Validates stream state, checks action validity, calls internal
   codec, updates stream positions/byte counts, manages state transitions based on return codes. */
...
/* XZ backdoor: replaces a function pointer in GOT/PLT by name. Changes memory protection, swaps
   pointer, restores protection. */
...
/* Executes callback on ready queue head item's function pointer, then clears it */
...
/* LZ decoder wrapper: buffers initial 13 bytes for LZMA header, then delegates to the inner decoder
   function pointer */

undefined8
lzma_lz_decoder_code
          (undefined8 *param_1,undefined8 param_2,undefined8 param_3,undefined8 param_4,
...
/* Block encoder filter update: returns error if finished flag set, otherwise delegates */
...
/**
 * @name  lzma_block_decode
 * @brief Decodes LZMA block with integrity check: state 0=decode, 1=padding, 2=check bytes.
 * @confidence 88%
 * @classification handler
 * @address 0x0010a830
 */

/* Decodes an LZMA block with integrity check handling. Operates in three states (0=decode,
   1=padding, 2=check). State 0 calls the filter chain to decompress data and updates checksums.
   State 1 writes padding zeros to align uncompressed size to 4 bytes. State 2 copies the integrity
   check bytes from the stream. Returns LZMA status codes. */

ulong lzma_block_decode(undefined8 *param_1,undefined8 param_2,long param_3,long *param_4,
                       long param_5,long param_6,ulong *param_7,ulong param_8,int param_9)

{
  ...
  lVar8 = *param_4;
  if (0x7fffffffffffffff - param_1[0xd] < (ulong)(param_5 - lVar8)) {
    return 9;
  }
  ...
  if (iVar1 == 1) {
    ...
    lVar2 = *param_4;
    uVar6 = *param_7;
    if (0x7ffffffffffffbbc - param_1[0xc] < uVar6 - uVar7) {
      return 9;
    }
    param_1[0xd] = param_1[0xd] + (lVar2 - lVar8);
    param_1[0xc] = (uVar6 - uVar7) + param_1[0xc];
    if (lVar2 - lVar8 != 0) {
      lzma_check_update(param_1 + 0xf,*(undefined4 *)(param_1[10] + 8),param_3 + lVar8);
  ...
  lzma_check_finish(param_1 + 0xf);
  *(undefined4 *)(param_1 + 0xb) = 2;
LAB_0010a9a4:
  uVar4 = lzma_check_size(*(undefined4 *)(param_1[10] + 8));
  uVar7 = (ulong)uVar4;
  lzma_bufcpy(param_1 + 0xf,param_1 + 0xe,uVar7,param_6,param_7,param_8);
  if ((ulong)param_1[0xe] < uVar7) {
    return 0;
  }
  memcpy((void *)(param_1[10] + 0x28),param_1 + 0xf,uVar7);
  return 1;
}
 ...
 * @brief State machine encoding LZMA index to output buffer. States: indicator, block count, sizes, padding, CRC32.
 * @confidence 85%
 * @classification handler
 * @address 0x0010b760
 */

/* State machine encoding LZMA index to output buffer. States: 0=index indicator, 1=block count VLI,
   2=unpadded size VLI, 3=uncompressed size VLI, 4=iterate records, 5=padding, 6=CRC32 checksum. */
 ...
 * @brief LZMA stream encoder state machine: stream header, start block/finish, block header, encode block, encode index, stream footer.
 * @confidence 88%
 * @classification handler
 * @address 0x0010bf70
 */

/* LZMA stream encoder state machine. States: 0=stream header, 1=start block/finish, 2=block header,
   3=encode block, 4=encode index, 5=stream footer. Handles block headers, compression, index
   appending, footer. */
        ...
        if (*param_4 == param_5) {
          ...
          *param_1 = 4;
          ...
          if ((char)param_1[1] == '\0') {
            ...
            if (((int)uVar4 != 0) ||
               (uVar4 = lzma_block_encoder_init(param_1 + 2,param_2,piVar1), (int)uVar4 != 0))
            goto LAB_0010c055;
          }
          *(undefined1 *)(param_1 + 1) = 0;
...
/* Thread-safe event signaling: sets event if none pending, signals condition variable */
 ...
 * @brief Worker thread for MT LZMA compression: waits for work, compresses blocks, falls back to uncompressed if larger, signals completion.
 * @confidence 85%
 * @classification handler
 * @address 0x0010cac0
 */

/* Worker thread for multi-threaded LZMA compression. Waits for work via condvars, compresses blocks
   with block encoder, falls back to uncompressed if larger, writes headers, signals completion to
   coordinator. */
  ...
  do {
    ...
    if (uVar10 < 3) {
      ...
      *(uint **)(param_1 + 0x2c) = param_1 + 0x58;
      ...
      if ((iVar5 != 0) ||
         (iVar5 = lzma_block_encoder_init(param_1 + 0x10,*(undefined8 *)(param_1 + 10),puVar1),
         iVar5 != 0)) {
LAB_0010ce40:
...
/* Signals MT decoder worker threads to stop (state 3), optionally waits for completion via
   condition variables. */

void lzma_mt_decoder_signal_workers(long param_1,char param_2)
 ...
 * @brief Multi-threaded LZMA stream encoder state machine: stream header, encode blocks with workers, write index, write footer.
 * @confidence 85%
 * @classification handler
 * @address 0x0010d750
 */

/* Multi-threaded LZMA stream encoder state machine. States: 0=stream header, 1=encode blocks with
   workers, 2=write index, 3=write footer. Manages worker creation, mutex/condvar coordination,
   block distribution. */
...
LAB_0010d7f0:
      ...
      uVar4 = param_1[0x69];
      if (uVar4 == 0) {
        uVar4 = lzma_outq_read(puVar1,param_2,param_6,param_7,param_8,&local_178,&local_170);
        pthread_mutex_unlock(__mutex);
        if (uVar4 == 1) {
LAB_0010d81a:
...
LAB_0010dda8:
      lzma_mt_decoder_signal_workers(param_1,0);
      goto LAB_0010dc80;
    }
    if (uVar4 != 3) {
      uVar4 = 0xb;
...
/* Simple filter decode process: invokes size calculator, calls inner decoder, applies bitwise NOT
   to output byte */

undefined8
lzma_simple_coder_decode
 ...
 * @brief LZMA2 decoder state machine: control byte, uncompressed size, compressed size, validate/init, delegate to LZMA1 decoder.
 * @confidence 85%
 * @classification handler
 * @address 0x0010e7c0
 */

/* LZMA2 decoder state machine. States: 0=control byte, 1=uncompressed size (4 bytes), 2=compressed
   size (8 bytes) with validation, 3=validate and init LZMA sub-decoder, 4=delegate to LZMA1
   decoder. */

undefined8
lzma_lzma2_decoder_code
          (undefined8 *param_1,undefined8 param_2,long param_3,ulong *param_4,ulong param_5,
      ...
      *param_4 = uVar5 + 1;
      ...
      if (7 < uVar8) {
        ...
        *(ulong *)((long)param_1 + 0xac) = uVar5;
        lVar6 = lzma_lzma_lclppb_decoder_memusage(param_1 + 0xf);
        param_1[0xb] = 0;
        uVar5 = lVar6 + 0x8000;
...
LAB_0010e96b:
      ...
      uVar4 = lzma_next_filter_update(param_1,param_2,&local_78);
 ...
 * @brief Auto-detect decoder: reads header byte (0xFD=xz, 'L'=lzip, other=alone), initializes appropriate decoder, then delegates.
 * @confidence 88%
 * @classification handler
 * @address 0x0010eca0
 */

/* Auto-detect decoder state machine: reads header byte (0xFD=xz, 'L'=lzip, other=alone),
   initializes appropriate decoder, then delegates. */

ulong lzma_auto_decoder_code
                (undefined8 *param_1,undefined8 param_2,long param_3,ulong *param_4,ulong param_5,
  ...
  if (iVar2 != 1) {
    ...
    *(undefined4 *)((long)param_1 + 0x5c) = 1;
    if (cVar1 == -3) {
      ...
      uVar3 = lzma_lzip_decoder_init();
      uVar4 = (uint)uVar3;
    }
    else {
      uVar3 = lzma_alone_decoder_coder_init();
      if ((int)uVar3 != 0) {
        return uVar3;
      }
 ...
 * @brief XZ block decoder: state 0=decode with size enforcement and check updating, state 1=padding, state 2=verify integrity check.
 * @confidence 88%
 * @classification handler
 * @address 0x0010f060
 */

/* XZ block decoder: state 0=decode block data with size enforcement and check updating, state
   1=read padding zeros (4-byte aligned), state 2=read and verify integrity check bytes. */

ulong lzma_block_decoder_code
...
LAB_0010f2f7:
      lzma_check_update(param_1 + 0x22,*(undefined4 *)(lVar12 + 8),param_6 + uVar1);
 ...
 * @brief Complex state machine for file info decoding: processes stream headers, index padding, stream footers, backward seeks, index decoding, multi-stream concatenation.
 * @confidence 75%
 * @classification handler
 * @address 0x00110140
 */

/* Complex state machine for file info decoding. Processes stream headers, index padding, stream
   footers, backward seeks, index decoding, multi-stream concatenation via lzma_index_cat. */
    ...
    if ((int)uVar5 == 0) {
      while( true ) {
        ...
        *(ulong *)(unaff_RBX + 0x54) = uVar8;
        if ((uVar8 < 0xc) && (uVar5 = lzma_lz_encoder_prepare(), (int)uVar5 != 0)) {
          return uVar5;
        }
        lVar4 = lzma_bufcpy();
        uVar8 = *(ulong *)(unaff_RBX + 0x56);
 ...
 * @brief State machine for decoding LZMA index: indicator, record count VLI, memory check, unpadded/uncompressed VLIs, padding, CRC32.
 * @confidence 88%
 * @classification handler
 * @address 0x00110890
 */

/* State machine for decoding LZMA index data. States: 0=indicator, 1=record count VLI, 2=memory
   check/init, 3/4=unpadded/uncompressed size VLIs, 5/6=padding, 7=CRC32 verification. */
 ...
 * @brief XZ stream decoder state machine: stream header, block header, decode block, index hash, stream footer, stream padding.
 * @confidence 88%
 * @classification handler
 * @address 0x001118c0
 */

/* XZ stream decoder state machine. States: 0=stream header, 1=block header, 2=decode block header,
   3=decode block, 4=index hash decode, 5=stream footer, 6=stream padding. Supports concatenated
   streams. */
 ...
 * @brief Thread-safe: locks mutex, sets finish flag, signals condvar
 * @confidence 85%
 * @classification handler
 * @address 0x00112280
 */

/* Thread-safe: locks mutex, sets finish flag, signals condvar */
 ...
 * @brief Completes a worker item: updates memory accounting, moves worker to free list, signals condition variable
 * @confidence 80%
 * @classification handler
 * @address 0x001122c0
 */

/* Completes a worker item: updates memory accounting, moves worker to free list, signals condition
   variable */
 ...
 * @brief MT decoder worker thread: waits for work via condvars, processes decode chunks in 0x4000-byte segments, signals completion.
 * @confidence 82%
 * @classification handler
 * @address 0x00112310
 */

/* Multi-threaded decoder worker thread function. Waits for work via condition variables, processes
   decode chunks in 0x4000-byte segments, signals completion to coordinator, handles error states
   and thread cleanup. */

undefined8 lzma_mt_decoder_worker_thread(int *param_1)
...
/* Signals all worker threads to state 2 by iterating, locking per-worker mutex, setting state, and
 ...
 * @brief Thread-safe decoder loop: processes input under mutex, waits for workers via condvar, signals workers when data ready. Returns 0x65 on timeout.
 * @confidence 78%
 * @classification handler
 * @address 0x001127b0
 */

/* Thread-safe decoder loop that processes input under mutex, waits for workers via
   pthread_cond_timedwait, manages thread pool coordination, signals workers when data is ready.
   Returns 0x65 on timeout. */

int lzma_mt_decoder_threaded_loop
...
LAB_00112830:
  do {
    lVar5 = *param_4;
    while (iVar4 = lzma_outq_read(lVar1,param_2,param_3,param_4,param_5,0,0), iVar4 == 1) {
 ...
 * @brief MicroLZMA decoder: validates header, decodes LZMA properties from first byte, initializes sub-decoder, dispatches to block decoding.
 * @confidence 82%
 * @classification handler
 * @address 0x00114180
 */

/* MicroLZMA decoder wrapper that validates header, decodes LZMA properties from first byte,
   initializes sub-decoder filter chain, then dispatches to block decoding with
   compressed/uncompressed size limit enforcement. */

undefined8
lzma_microlzma_decoder_code
          (undefined8 *param_1,undefined8 param_2,long param_3,ulong *param_4,ulong param_5,
 ...
 * @brief Lzip format decoder state machine: magic, version, dictionary size, init LZMA, decompress, verify trailer (CRC32/sizes).
 * @confidence 85%
 * @classification handler
 * @address 0x00114660
 */

/* LZIP format decoder state machine. States: 0=magic 'LZIP', 1=version, 2=dictionary size, 3=init
   LZMA, 4=decompress, 5=verify trailer (CRC32, sizes). Supports v0/v1 with different trailer sizes.
    */

ulong lzma_lzip_decoder_code
                (undefined4 *param_1,undefined8 param_2,long param_3,ulong *param_4,ulong param_5,
...
LAB_00114a08:
  ...
  *(undefined8 *)(param_1 + 0x1c) = 0x200000000;
  lVar7 = lzma_lzma_lclppb_decoder_memusage(param_1 + 0x16);
  *param_1 = 3;
  uVar3 = lVar7 + 0x8000;
...
LAB_001146c4:
  ...
  local_60 = (undefined1  [16])0x0;
  uVar5 = lzma_next_filter_update(param_1 + 0x32,param_2,&local_78);
  uVar3 = (ulong)uVar5;
  if (uVar5 != 0) {
code_r0x001148c9:
    if (local_40 == *(long *)(in_FS_OFFSET + 0x28)) {
...
/**
 * @name  lzma_lz_decoder_code
 * @brief LZ decoder processing loop with internal buffering. Manages input-to-dictionary copying, dispatches to sub-decoder, handles wraparound.
 * @confidence 85%
 * @classification handler
 * @address 0x00116610
 */

/* LZ decoder processing loop with internal buffering. Manages input-to-dictionary copying,
   dispatches to sub-decoder, handles buffer wraparound with memmove, and applies post-decode
   callback. */

ulong lzma_lz_decoder_code
...
/**
 * @name  lzma_buffered_decoder_process
 * @brief Buffered decoder wrapping inner decoder. Manages circular output buffer with 0x120-byte overlap for continued decoding.
 * @confidence 82%
 * @classification handler
 * @address 0x001181f0
 */

/* Buffered decoder that wraps an inner decoder. Manages a circular output buffer, calls the inner
   decoder to fill it, then copies decoded data to the output. When buffer is exhausted, shifts the
   last 0x120 bytes to the front as overlap/dictionary for continued decoding. */

int lzma_buffered_decoder_process
...

{
  ...
  uVar6 = param_1[4];
  do {
    if (uVar5 == uVar6) goto LAB_001182c9;
    while( true ) {
      uVar3 = param_7 - *param_6;
      if (uVar6 - uVar5 < (ulong)(param_7 - *param_6)) {
        uVar3 = uVar6 - uVar5;
      }
      param_1[3] = uVar3 + uVar5;
      iVar2 = (*(code *)param_1[7])(param_1[6],param_1,param_2,param_3,param_4);
      lVar4 = *param_6;
      __n = param_1[1] - uVar5;
      if (__n != 0) {
        memcpy((void *)(param_5 + lVar4),(void *)(*param_1 + uVar5),__n);
      }
      lVar4 = __n + lVar4;
      cVar1 = *(char *)((long)param_1 + 0x29);
      *param_6 = lVar4;
      if (cVar1 == '\0') break;
      param_1[1] = 0x240;
      param_1[2] = 0;
...
LAB_001182c9:
        ...
        *(undefined1 *)(param_1 + 5) = 1;
        puVar7 = (undefined8 *)((long)puVar8 + (uVar5 - 0x120));
        *puVar8 = *puVar7;
        puVar8[0x23] = puVar7[0x23];
        lVar4 = (long)puVar8 - (long)((ulong)(puVar8 + 1) & 0xfffffffffffffff8);
        puVar7 = (undefined8 *)((long)puVar7 - lVar4);
        puVar8 = (undefined8 *)((ulong)(puVar8 + 1) & 0xfffffffffffffff8);
        for (uVar5 = (ulong)((int)lVar4 + 0x120U >> 3); uVar5 != 0; uVar5 = uVar5 - 1) {
          *puVar8 = *puVar7;
...
/**
 * @name  lzma_lzma2_decoder_code
 * @brief Two-stage buffered decoder: fills internal buffer from inner decoder, then decodes to output.
 * @confidence 82%
 * @classification handler
 * @address 0x00118380
 */

/* Two-stage buffered decoder: fills 0x1000-byte internal buffer from inner decoder, then decodes to
   output. */

ulong lzma_lzma2_decoder_code
 ...
 * @brief LZMA range encoder: encodes literals/matches/reps via range coding with probability-based bit encoding, carry propagation, distance/length encoding, EOS marker.
 * @confidence 85%
 * @classification handler
 * @address 0x00118c60
 */

/* LZMA range encoder output function. Encodes literals/matches/reps via range coding with
   probability-based bit encoding. Manages range normalization, carry propagation, distance/length
   encoding, EOS marker. */

undefined8
lzma_lzma_encoder_encode
          (ulong *param_1,long *param_2,long param_3,long *param_4,long param_5,uint param_6)
...
LAB_00118e33:
      if (*(char *)((long)param_1 + 0xb74) == '\0') {
        lzma_lzma_optimal_match(param_1,param_2,&local_44,&local_48,(int)param_1[0x56]);
      }
      else {
        lzma_lzma_find_match_or_literal(param_1,param_2,&local_44,&local_48,0x800);
      }
      uVar23 = local_48;
      uVar18 = param_1[0x56];
        ...
        *(undefined4 *)((long)param_1 + uVar11 * 4 + 0x30) = 1;
        ...
        if (local_44 < 4) {
          ...
          *(uint *)(param_1 + 0x59) = (-(uint)(uVar19 < 7) & 0xfffffffd) + 10;
          ...
          if (uVar8 < 0x2000) {
            uVar19 = (uint)(byte)(&log2_slot_table)[uVar8];
          }
          else if (uVar8 < 0x2000000) {
            uVar19 = (byte)(&log2_slot_table)[uVar8 >> 0xc] + 0x18;
          }
          else {
            uVar19 = (byte)(&log2_slot_table)[uVar8 >> 0x18] + 0x30;
          }
          uVar14 = param_1[4];
          uVar18 = 1;
          uVar7 = 5;
...
/* Validates and updates LZMA encoder filter lc/lp/pb configuration from a filter pair structure */
 ...
 * @brief LZMA2 block encoding state machine: check completion, compress, write block header, write uncompressed header, copy uncompressed.
 * @confidence 85%
 * @classification handler
 * @address 0x00120860
 */

/* LZMA2 block buffer encoding state machine. States: 0=check completion, 1=compress data, 2=write
   block header, 3=write uncompressed header, 4=copy uncompressed data. Handles compressed and
   uncompressed paths. */

ulong lzma_lzma2_encoder_encode
...
switchD_001208a7_caseD_0:
  ...
  if (*(char *)((long)param_1 + 0x81) != '\0') {
    uVar7 = lzma_lzma_encoder_reset(uVar9,param_1 + 4);
    if ((int)uVar7 != 0) {
      return uVar7;
    }
    iVar16 = (int)param_2[3];
...
LAB_001209a1:
  ...
  iVar11 = lzma_lzma_encoder_encode
                     (uVar9,param_2,(long)param_1 + 0xa6,param_1 + 0x24,0x10000,iVar11);
 ...
 * @brief Decodes LZMA2 compressed blocks through state machine: type byte, uncompressed size, compressed size, filter properties, decompress, copy.
 * @confidence 85%
 * @classification handler
 * @address 0x00120f30
 */

/* Decodes LZMA2 compressed blocks through state machine: 0=type byte, 1-2=uncompressed size,
   3-4=compressed size, 5=filter properties, 6=decompress, 7=copy uncompressed. Used internally by
   LZMA2 decoder. */

undefined8
lzma_lzma2_decoder_decode
          (int *param_1,undefined8 *param_2,long param_3,ulong *param_4,ulong param_5)
 ...
 * @brief Delta decoder: XOR-subtracts values from 256-byte history table, supports callback and direct copy modes.
 * @confidence 85%
 * @classification handler
 * @address 0x001214b0
 */

/* Stream cipher/delta decoder: XOR-subtracts values from 256-byte substitution table, supports
   callback and direct copy modes. */

ulong lzma_delta_decoder_code
                (undefined8 *param_1,undefined8 param_2,long param_3,long *param_4,long param_5,
 ...
 * @brief Delta encoder processing: calls inner coder, then XOR-subtracts values from 256-byte substitution table
 * @confidence 80%
 * @classification handler
 * @address 0x00121660
 */

/* Delta encoder processing: calls inner coder, then XOR-subtracts values from 256-byte substitution
   table */

void lzma_delta_encoder_code(undefined8 *param_1)
 ...
 * @brief Processes one step of simple coder (BCJ filter wrapper). If no sub-coder, does buffer copy; otherwise delegates
 * @confidence 80%
 * @classification handler
 * @address 0x001217c0
 */

/* Processes one step of simple coder (BCJ filter wrapper). If no sub-coder, does buffer copy;
   otherwise delegates */

undefined8
lzma_simple_coder_code
          (undefined8 *param_1,undefined8 param_2,undefined8 param_3,long *param_4,long param_5,
          undefined8 param_6,undefined8 param_7,undefined8 param_8,int param_9)
 ...
 * @brief Decodes LZMA stream with internal output buffering for post-processing (BCJ filter application on output).
 * @confidence 80%
 * @classification handler
 * @address 0x00121850
 */

/* Decodes LZMA stream with internal output buffering for post-processing (integrity check). Manages
   partial reads from internal buffer, dispatches to decode process, applies post-processing
   callback on output. */

ulong lzma_simple_coder_decode_with_buffering
                (long param_1,undefined8 param_2,undefined8 param_3,undefined8 param_4,
...
LAB_001218da:
    uVar3 = lzma_simple_coder_code
                      (param_1,param_2,param_3,param_4,param_5,__dest,param_1 + 0x88,
 ...
 * @brief Parses LZMA compression options from a comma-separated string of name=value pairs. Supports three value types: presets (digit + optional 'e' extreme flag), string enums (looked up from a name-value table), and integers with optional KiB/MiB/GiB suffixes. Validates ranges and stores parsed values into the options structure at descriptor-specified offsets. Returns NULL on success or an error string on failure.
 * @confidence 92%
 * @classification parser
 * @address 0x001074e0
 */

/* Parses LZMA compression options from a comma-separated string of name=value pairs. Supports three
   value types: presets (digit + optional 'e' extreme flag), string enums (looked up from a
   name-value table), and integers with optional KiB/MiB/GiB suffixes. Validates ranges and stores
   parsed values into the options structure at descriptor-specified offsets. Returns NULL on success
   or a static error string on failure. Part of liblzma's lzma_str_to_filters implementation. */
...
/* Sets default flags value 0x100000000 then delegates to option string parser */

void lzma_str_to_filters_with_default(undefined8 param_1,undefined8 param_2,undefined8 *param_3)
 ...
 * @brief Public API that delegates to internal LZMA options string parser
 * @confidence 80%
 * @classification parser
 * @address 0x001078b0
 */

/* Public API that delegates to internal LZMA options string parser */

void lzma_str_to_filters(void)
...
/* Validates LZMA preset string options, returns error message if lc+lp > 4 */
...
/**
 * @name  lzma_str_to_filters
 * @brief Parses string representation of LZMA filter chain into filter array. Supports preset syntax and named filters with options.
 * @confidence 88%
 * @classification parser
 * @address 0x00107ab0
 */
...
/* Parses string representation of LZMA filter chain into filter array. Supports preset syntax (e.g.
   '6', '6e') and named filters with options. Max 4 filters. Returns NULL on success or error
   string. */
 ...
 * @brief Parses ELF dynamic section extracting DT_SYMTAB, DT_STRTAB, DT_JMPREL, etc. Allocates 64-byte result structure.
 * @confidence 85%
 * @classification parser
 * @address 0x00108b10
 */

/* Parses ELF dynamic section extracting DT_SYMTAB, DT_SYMENT (must be 0x18), DT_STRTAB, DT_STRSZ,
   DT_JMPREL, DT_PLTRELSZ, DT_REL/RELA, DT_RELSZ/RELASZ, DT_RELENT/RELAENT. Allocates 64-byte result
   structure. */
 ...
 * @brief Searches ELF section header arrays (types 6=SHT_DYNAMIC, 7=SHT_NOTE) for string table entries, returns name pointer and address.
 * @confidence 82%
 * @classification parser
 * @address 0x001090d0
 */

/* Searches ELF section header arrays (types 6/7) for string table entries, returns name pointer and
   address. */
...
/* Encodes LZMA block header: writes size byte, compressed/uncompressed VLIs, filter flags, padding
   zeros, CRC32. */
...
/* Encodes LZMA filter properties by searching filter table for matching ID and calling encoder */
...
/* Encodes LZMA filter flags: writes filter ID as VLI, properties size as VLI, then the properties
   themselves */
...
/* Single-call index encoding: validates params, checks buffer space, initializes iterator, encodes
   via helper */
 ...
 * @brief Encodes XZ stream header: writes 6-byte magic, stream flags byte, and CRC32
 * @confidence 95%
 * @classification parser
 * @address 0x0010c670
 */

/* Encodes XZ stream header: writes magic bytes, stream flags, and CRC32 */
...

{
  ...
  *(undefined2 *)(param_2 + 1) = xz_stream_header_magic_suffix;
 ...
 * @brief Encodes LZMA stream footer: validates fields, writes backward size and check type, computes CRC32, writes magic bytes
 * @confidence 92%
 * @classification parser
 * @address 0x0010c700
 */

/* Encodes an LZMA stream footer: validates fields, writes backward size and check type, computes
   CRC32, writes magic bytes */
 ...
 * @brief Encodes variable-length integer: 7 data bits per byte, bit 7 as continuation flag.
 * @confidence 92%
 * @classification parser
 * @address 0x0010c7b0
 */

/* Encodes variable-length integer: 7 data bits per byte, bit 7 as continuation flag. Supports
 ...
 * @brief Decodes LZMA block header: validates size/CRC-32, decodes compressed/uncompressed sizes and filter flags.
 * @confidence 90%
 * @classification parser
 * @address 0x0010f580
 */

/* Decodes LZMA block header from raw bytes. Validates header size/check, verifies CRC-32, decodes
   optional compressed/uncompressed sizes (VLI), decodes filter flags, verifies zero padding, frees
   on error. */
...
/* Resumable state machine that reads an .xz file backward to parse stream footers, block padding,
   indices, and stream headers without decompressing. Handles multi-stream .xz files by seeking
   backward through concatenated streams and merging their indices via lzma_index_cat. States:
   0=read initial stream header, 1=begin backward scan, 2=read+scan padding, 3=read stream footer,
   4=seek to index, 5=decode index, 6=read stream header, 7=compare stream flags. Returns LZMA_OK(0)
   when more input needed, LZMA_SEEK_NEEDED(12) when seek required, LZMA_DATA_ERROR(9) on
   corruption, LZMA_STREAM_END(1) when done. */
...

{
  ...
  do {
    *param_1 = 2;
    uVar3 = lzma_lz_encoder_prepare(param_1,lVar1,param_4,param_5);
    if ((int)uVar3 != 0) {
      return uVar3;
    }
...
/* Decodes LZMA filter properties by searching 12 filter types in jump table, dispatching to
   appropriate decoder */
...

{
  ...
  while( true ) {
    if (*param_1 == lVar3) {
      if ((code *)(&PTR_lzma_lzma_props_decode_00131c38)[lVar1 * 4] != (code *)0x0) {
                    /* WARNING: Could not recover jumptable at 0x00110721. Too many branches */
                    /* WARNING: Treating indirect jump as call */
        lVar3 = (*(code *)(&PTR_lzma_lzma_props_decode_00131c38)[lVar1 * 4])(param_1 + 1);
        return lVar3;
      }
...
/* Decodes LZMA filter flags: reads filter ID VLI, properties size VLI, then decodes properties */
...
/* State machine dispatch for index hash decoding: processes input byte-by-byte with CRC32
   computation */

undefined8 switchD_001108f4::lzma_index_hash_decode_dispatch(undefined8 param_1,long param_2)
...
/* Single-call index decoding from buffer: validates params, initializes decoder, runs state
   machine, handles errors. */
...
/* Incrementally decodes and validates an LZMA Index block with hash verification. Uses a 7-state
   ...
   bytes are zero, (6) verify trailing CRC32. Returns LZMA_OK when more input needed,
   LZMA_STREAM_END on success, LZMA_DATA_ERROR on mismatch. */
...
/* Processes a block of data for CRC32 checksum computation during index hash decoding with state
   dispatch */

void switchD_00111145::lzma_index_hash_decode_crc32(undefined8 param_1,long param_2)
...
/* Decodes LZMA stream header: validates 6-byte magic, checks CRC32, extracts check type from flags
   byte */
...

{
  ...
  if ((short)param_2[1] == xz_stream_header_magic_suffix) {
...
/* Decodes LZMA stream footer: validates magic bytes, CRC32, reserved fields, extracts check type
   and backward size */
...
/* Decodes variable-length integer from input buffer. Each byte has 7 data bits, bit 7 is
   continuation. Supports incremental decoding. */
 ...
 * @brief Main decode function for multi-threaded XZ stream decoder. Implements a 12-state machine (states 0-11) parsing the .xz stream structure: stream header → block header parsing → memory check → worker thread dispatch → threaded block decode → single-thread fallback → single-thread block decode → wait for workers → index hash decode → stream footer → stream padding → error state. Manages a pthread worker pool, output queue, and memory limits. Falls back to single-threaded decoding when memory constraints prevent parallel operation. The state variable at *coder (offset 0) persists across calls for incremental decoding.
 * @confidence 95%
 * @classification parser
 * @address 0x00112af0
 */

/* Main decode function for multi-threaded XZ stream decoder. Implements a 12-state machine parsing
   .xz stream structure: stream header (0) → block header parsing (1) → memory check (2) →
   worker thread dispatch (3) → threaded block decode (4) → single-thread fallback memory wait
   (5) → single-thread block decode (6) → wait for workers before index (7) → index hash
   decode (8) → stream footer (9) → stream padding (10) → error state (11). Manages pthreads
   worker pool, output queue, and memory limits. Falls back to single-threaded decoding when memory
   constraints prevent parallel operation. */
...
LAB_0011310f:
        ...
        uVar27 = lzma_mt_decoder_threaded_loop
...
switchD_00112b88_caseD_5:
        ...
        if ((int)uVar27 == 0) {
          if (param_1[0x86] == 0) {
            ...
            lzma_mt_decoder_stop_workers(param_1,param_2);
...
LAB_001139d8:
      ...
      *(undefined1 (*) [16])(puVar30 + 0x28) = (undefined1  [16])0x0;
      sigfillset((sigset_t *)local_c8);
      pthread_sigmask(2,(__sigset_t *)local_c8,&local_148);
      iVar29 = pthread_create((pthread_t *)(puVar30 + 0x7c),(pthread_attr_t *)0x0,
                              lzma_mt_decoder_worker_thread,puVar30);
      pthread_sigmask(2,&local_148,(__sigset_t *)0x0);
      if (iVar29 == 0) {
...
/* Encodes lc, lp, pb values into single byte: pb*45 + lp*9 + lc */
 ...
 * @brief Encodes LZMA properties byte (lc/lp/pb) and dictionary size into 5-byte buffer
 * @confidence 92%
 * @classification parser
 * @address 0x0011a860
 */

/* Encodes LZMA properties byte (lc/lp/pb) and dictionary size into 5-byte buffer */

undefined8 lzma_lzma_props_encode(undefined4 *param_1,char *param_2)

{
  ...
  uVar1 = param_1[5];
  uVar3 = 0xb;
  if (((uVar1 < 5) && (uVar2 = param_1[6], uVar2 < 5)) && (uVar1 + uVar2 < 5)) {
    if (4 < (uint)param_1[7]) {
      return uVar3;
    }
    *param_2 = ((char)param_1[7] * '\x05' + (char)uVar2) * '\t' + (char)uVar1;
...
/* Core LZMA range-coded decoder implementing the full LZMA decode loop. Uses adaptive probability
   models with 11-bit precision (0x800 initial). Supports streaming via a 23-state resumption
   machine (coder+0x6ea4). Contains a slow path with boundary checks and an optimized fast inner
   loop with unrolled 8-iteration literal decoding. Decodes literals, matches, short reps (rep0),
   and long reps (rep0-rep3) with distance slot decoding via bit-tree coded probabilities and
   reverse bit-tree alignment bits. The 0x408010 constant is a bit-table for checking which decoder
   states represent valid end-of-stream positions. */
    ...
    if (uVar28 < 0x1000000) {
      if (pbVar1 == pbVar41) {
        *(undefined4 *)(param_1 + 0x6ea4) = 0xd;
        uVar18 = lzma_lzma_encoder_store_state();
        return uVar18;
      }
      bVar4 = *pbVar41;
      uVar28 = uVar28 << 8;
 ...
 * @brief Decodes single byte into lc, lp, pb LZMA properties using division by 45 and 9
 * @confidence 92%
 * @classification parser
 * @address 0x00120490
 */

/* Decodes single byte into lc, lp, pb LZMA properties */

undefined4 lzma_lzma_lclppb_decode(long param_1,byte param_2)

{
  ...
  uVar3 = 1;
  if (param_2 < 0xe1) {
    *(uint *)(param_1 + 0x1c) = (uint)(param_2 / 0x2d);
    bVar1 = (byte)((uint)(param_2 % 0x2d) * 0x39 >> 8) >> 1;
    *(uint *)(param_1 + 0x18) = (uint)bVar1;
    iVar4 = (uint)(param_2 % 0x2d) - ((uint)bVar1 + (uint)bVar1 * 8);
    uVar2 = (uint)bVar1 + iVar4;
    *(int *)(param_1 + 0x14) = iVar4;
    uVar3 = CONCAT31((int3)(uVar2 >> 8),4 < uVar2);
  }
  return uVar3;
}
...
/* Decodes LZMA properties byte: extracts lc, lp, pb values from encoded byte, allocates 0x70-byte
   options structure */

undefined8 lzma_lzma_props_decode(undefined8 *param_1,undefined8 param_2,byte *param_3,long param_4)
...
/**
 * @name  lzma_lzma_dict_size_encode
...
/* Computes next power of two for dictionary size and maps to a log2 table index for LZMA encoding
   parameter */

undefined8 lzma_lzma_dict_size_encode(uint *param_1,char *param_2)

{
  ...
  uVar1 = uVar1 + 1;
  if (uVar1 < 0x2000) {
    *param_2 = (&log2_slot_table)[uVar1] - 0x18;
    return 0;
  }
  if (0x1ffffff < uVar1) {
    *param_2 = (&log2_slot_table)[uVar1 >> 0x18] + '\x18';
    return 0;
  }
  *param_2 = (&log2_slot_table)[uVar1 >> 0xc];
  return 0;
}
...
/* Encodes simple filter properties: validates then writes start_offset-1 */
...
/* Decodes single byte simple filter properties, allocates 0x28-byte options struct */

undefined8
lzma_simple_props_decode(undefined8 *param_1,undefined8 param_2,byte *param_3,long param_4)
...
/* Copies delta filter property value if source is valid */
...
/**
 * @name  bcj_x86_filter
 * @brief x86 BCJ filter detecting E8 (CALL) and E9 (JMP) instructions, converts relative addresses with probability-based state machine.
 * @confidence 92%
 * @classification parser
 * @address 0x00121dc0
 */

/* x86 BCJ (Branch-Call-Jump) filter that detects E8 (CALL) and E9 (JMP) instructions and converts
   their relative addresses between absolute and relative forms for improved compression. Uses a
   probability-based state machine to decide whether to transform each instruction. */

ulong decode_x86_branch_instructions
                (uint *param_1,int param_2,char param_3,long param_4,ulong param_5)

{
  ...
  do {
    uVar13 = uVar12 + 1;
    if ((byte)(*(char *)(param_4 + uVar12) + 0x18U) < 2) {
      uVar10 = param_2 + (int)uVar12;
      pbVar1 = (byte *)(param_4 + 4 + uVar12);
      bVar4 = *pbVar1;
      bVar9 = bVar4 - 1;
      if (uVar10 - uVar8 < 6) {
        if (uVar10 != uVar8) {
        ...
        if (bVar9 < 0xfe) {
          ...
          uVar8 = uVar6 >> 1;
          if ((uVar6 < 10) && (uVar8 != 3)) goto LAB_00121eaf;
          uVar6 = uVar6 | 0x11;
          uVar8 = uVar10;
        }
      }
        ...
        if (0xfd < bVar9) {
          ...
          uVar8 = 0;
LAB_00121eaf:
          pbVar2 = (byte *)(param_4 + 3 + uVar12);
          pbVar11 = (byte *)(uVar13 + param_4);
          pbVar3 = (byte *)(param_4 + 2 + uVar12);
          uVar7 = (uint)*pbVar2 << 0x10 | (uint)bVar4 << 0x18 | (uint)*pbVar11 | (uint)*pbVar3 << 8;
          while( true ) {
            iVar5 = uVar10 + 5;
            if (param_3 == '\0') {
              iVar5 = -5 - uVar10;
            ...
            uVar7 = iVar5 + uVar7;
            if ((uVar6 == 0) ||
               ((byte)((char)(uVar7 >> (('\x03' - (char)(&DAT_0012adc0)[uVar8]) * '\b' & 0x1fU)) -
                      1U) < 0xfe)) break;
            uVar7 = (1 << (('\x04' - (char)(&DAT_0012adc0)[uVar8]) * '\b' & 0x1fU)) - 1U ^ uVar7;
          }
          uVar13 = uVar12 + 5;
          *pbVar1 = -((byte)(uVar7 >> 0x18) & 1);
          *pbVar2 = (byte)(uVar7 >> 0x10);
          *pbVar3 = (byte)(uVar7 >> 8);
          *pbVar11 = (byte)uVar7;
          uVar6 = 0;
          uVar8 = uVar10;
...
/* PowerPC BCJ filter: detects branch instructions (opcode 0x48 pattern) and adjusts 26-bit relative
   addresses */

long bcj_powerpc_filter(undefined8 param_1,int param_2,char param_3,long param_4,ulong param_5)
 ...
 * @brief ARM BCJ filter: processes 16-byte blocks, uses lookup table for instruction identification, adjusts 21-bit relative offsets.
 * @confidence 85%
 * @classification parser
 * @address 0x00122140
 */

/* Iterates over 16-byte blocks of encoded data, using a lookup table to identify branch-like
   instruction patterns, and adjusts relative offset fields based on the current position. Likely an
   ARM/RISC branch instruction filter for compression (BCJ-like). */

ulong patch_branch_instructions
...

{
  ...
  if (param_5 < 0x10) {
    ...
    do {
      ...
      uVar12 = 0;
      uVar11 = 5;
      uVar3 = *(uint *)(&DAT_0012ade0 + (ulong)(*(byte *)(param_4 - 0x10 + uVar15) & 0x1f) * 4);
      iVar16 = param_2 - 0x10 + (int)uVar15;
      do {
        if ((uVar3 >> (uVar12 & 0x1f) & 1) != 0) {
          lVar7 = 0;
          uVar14 = 0;
          bVar10 = (byte)uVar11 & 7;
          lVar2 = param_4 + (ulong)(uVar11 >> 3) + (uVar15 - 0x10);
          do {
            pbVar1 = (byte *)(lVar2 + lVar7);
            cVar5 = (char)lVar7;
            lVar7 = lVar7 + 1;
            uVar14 = uVar14 + ((ulong)*pbVar1 << (cVar5 * '\b' & 0x3fU));
          } while (lVar7 != 6);
          uVar8 = uVar14 >> bVar10;
          uVar4 = (uint)(uVar8 >> 0x20);
          if (((uVar4 >> 5 & 0xf) == 5) &&
             (uVar9 = (ulong)((uint)(uVar8 >> 9) & 7), (uVar8 >> 9 & 7) == 0)) {
            iVar6 = -iVar16;
            if (param_3 != '\0') {
              iVar6 = iVar16;
            }
            uVar13 = (ulong)(((uVar4 >> 4 & 1) << 0x14 | (uint)(uVar8 >> 0xd) & 0xfffff) * 0x10 +
                             iVar6 >> 4);
            do {
              *(char *)(lVar2 + uVar9) =
                   (char)((uVar14 & (1 << bVar10) - 1 |
                          (uVar8 & 0xffffffee00001fff |
                          (uVar13 & 0xfffff) << 0xd | (uVar13 & 0x100000) << 0x10) << bVar10) >>
                         ((char)uVar9 * '\b' & 0x3fU));
              uVar9 = uVar9 + 1;
            } while (uVar9 != 6);
          }
...
/* SPARC BCJ filter: processes 4-byte aligned instructions, detects opcode 0xeb pattern and adjusts
   relative branch targets */

long bcj_sparc_filter(undefined8 param_1,int param_2,char param_3,byte *param_4,ulong param_5)
 ...
 * @brief ARM Thumb BCJ filter for BL/BLX instruction pairs with 0xF0/0xF8 patterns, converts relative branch addresses.
 * @confidence 92%
 * @classification parser
 * @address 0x00122480
 */

/* ARM Thumb BCJ filter: processes 2-byte aligned instructions with BL/BLX patterns (0xF0/0xF8),
   adjusts 21-bit offsets. */

long bcj_armthumb_filter(undefined8 param_1,int param_2,char param_3,long param_4,ulong param_5)
...
/**
 * @name  bcj_arm64_filter
 * @brief ARM64/AArch64 BCJ filter: adjusts BL (branch-link) and ADRP instructions with offset fields
 * @confidence 90%
 * @classification parser
 * @address 0x00122600
 */

/* ARM64/AArch64 BCJ filter: adjusts BL (branch-link) and ADRP instructions with offset fields */

long bcj_arm64_filter(undefined8 param_1,int param_2,char param_3,long param_4,ulong param_5)
 ...
 * @brief RISC-V BCJ filter detecting AUIPC (0x17) and JAL (0x6F) patterns, converts relative branch offsets for compression.
 * @confidence 88%
 * @classification parser
 * @address 0x00122780
 */

/* RISC-V BCJ filter: detects AUIPC (0x17) and JAL (0x6F) patterns with specific opcode bits,
   relocates 22-bit offsets. */

long bcj_riscv_filter(undefined8 param_1,int param_2,char param_3,long param_4,ulong param_5)
...
/**
 * @name  bcj_riscv_decode_filter
 * @brief BCJ decode filter for RISC-V: converts between relative and absolute branch/call addresses for AUIPC and JAL-like instruction pairs.
 * @confidence 85%
 * @classification parser
 * @address 0x001228f0
 */

/* BCJ filter for RISC-V architecture that converts between relative and absolute branch/call
   addresses. Processes instruction pairs with specific opcode patterns (0xEF for AUIPC-like, 0x17
   for branch-like) for improved compression. */

void bcj_riscv_filter(undefined8 param_1,int param_2,undefined8 param_3,long param_4,ulong param_5)
 ...
 * @brief ARM64 BCJ decode filter - inverse of bcj_arm64_filter. Converts absolute addresses back to relative for ADRP+ADD and BL.
 * @confidence 85%
 * @classification parser
 * @address 0x00122af0
 */

/* ARM64 BCJ decode filter - inverse of bcj_arm64_filter. Converts absolute addresses back to
   relative for ADRP+ADD and BL instruction pairs during decompression. */

void bcj_arm64_decode_filter
 ...
 * @brief Returns number of available CPU threads via sched_getaffinity and __sched_cpucount
 * @confidence 95%
 * @classification utility
 * @address 0x00105030
 */

/* Returns number of available CPU threads via sched_getaffinity */
...
/* Copies data between buffers with dual position tracking, returns bytes copied */
 ...
 * @brief Gets compression progress from stream state via callback or direct field read
 * @confidence 90%
 * @classification utility
 * @address 0x00105730
 */

/* Gets compression progress from stream state via callback or direct field read */

void lzma_get_progress(long param_1,undefined8 *param_2,undefined8 *param_3)

{
  code *UNRECOVERED_JUMPTABLE;
  
  UNRECOVERED_JUMPTABLE = (code *)(*(undefined8 **)(param_1 + 0x38))[5];
  if (UNRECOVERED_JUMPTABLE != (code *)0x0) {
                    /* WARNING: Could not recover jumptable at 0x00105744. Too many branches */
                    /* WARNING: Treating indirect jump as call */
    (*UNRECOVERED_JUMPTABLE)(**(undefined8 **)(param_1 + 0x38));
    return;
  }
...
/**
 * @name  lzma_get_check
 * @brief Gets integrity check type from LZMA stream via get_check callback at offset [6]
 * @confidence 90%
 * @classification utility
 * @address 0x00105760
 */

/* Gets integrity check type from LZMA stream via get_check callback */

undefined8 lzma_get_check(long param_1)

{
  code *UNRECOVERED_JUMPTABLE;
  undefined8 uVar1;
  
  UNRECOVERED_JUMPTABLE = (code *)(*(undefined8 **)(param_1 + 0x38))[6];
  if (UNRECOVERED_JUMPTABLE != (code *)0x0) {
                    /* WARNING: Could not recover jumptable at 0x00105774. Too many branches */
                    /* WARNING: Treating indirect jump as call */
    uVar1 = (*UNRECOVERED_JUMPTABLE)(**(undefined8 **)(param_1 + 0x38));
    return uVar1;
  }
  return 0;
}
...
/* Queries current memory usage of an LZMA stream by calling the memconfig function pointer */
...
/* Gets the current memory limit of an LZMA stream via memconfig callback */
...
/* Sets memory limit for LZMA stream. Treats 0 as 1 (minimum). Calls memconfig callback */
...
/* Calculates unpadded size of an LZMA block from header, compressed, and check sizes with overflow
   checks */
...
/* Calculates compressed size of a block from unpadded size, header size, and check size */
...
/* Validates a filter chain: checks each filter ID against allowed types, verifies chain constraints
   (max 4 filters, ordering rules) */

undefined8 lzma_validate_filter_chain(long *param_1,ulong *param_2)
 ...
 * @brief Deep copies LZMA filter chain: iterates up to 4 filters, looks up options size, allocates and copies options. Frees on failure.
 * @confidence 88%
 * @classification utility
 * @address 0x00105b20
 */

/* Deep copies LZMA filter chain. Iterates up to 4 filters, looks up options size, allocates and
   copies options. Frees allocated options on failure. Uses optimized memcpy for final copy. */
...
/* Validates filter pointer then delegates to internal memusage calculation */
...

{
  ...
  if ((param_1 != (long *)0x0) && (*param_1 != -1)) {
    uVar1 = lzma_validate_filter_chain();
    return uVar1;
  }
  return 0xb;
}
...
/* Calculates total memory usage for a raw coder by iterating filters, calling finder for each,
   summing results */

long lzma_raw_coder_memusage(code *param_1,long *param_2)

{
  ...
  if ((param_2 != (long *)0x0) && (*param_2 != -1)) {
    iVar1 = lzma_validate_filter_chain(param_2,auStack_28);
    if (iVar1 == 0) {
      lVar3 = *param_2;
      lVar4 = 0;
      param_2 = param_2 + 1;
 ...
 * @brief Inserts a node into an implicit binary tree structure used by LZMA index, rebalances by rotating nodes based on count.
 * @confidence 88%
 * @classification utility
 * @address 0x00106220
 */

/* Inserts a node into an implicit binary tree structure, rebalancing by rotating nodes based on
   count. */

void lzma_index_tree_append(long *param_1,long param_2)
...
/**
 * @name  lzma_index_stream_accumulate
 * @brief Recursively accumulates statistics from tree nodes into accumulator structure
 * @confidence 80%
 * @classification utility
 * @address 0x001062c0
 */

/* Recursively accumulates statistics from tree nodes into accumulator */

void lzma_index_stream_accumulate(long *param_1,long *param_2)

{
  ...
  do {
    ...
    if (param_2[3] != 0) {
      lzma_index_stream_accumulate(param_1);
    }
    *param_2 = *param_2 + *param_1;
    param_2[1] = param_2[1] + param_1[1];
    *(int *)(param_2 + 5) = (int)param_2[5] + (int)param_1[3];
    param_2[6] = param_2[6] + param_1[2];
    lzma_index_tree_append(param_1[4],param_2);
    param_2 = plVar1;
...
/**
 * @name  lzma_index_iter_set_info
 * @brief Populates iterator output fields from current stream/group/record position in the index tree.
 * @confidence 72%
 * @classification utility
 * @address 0x00106320
 */

void FUN_00106320(long *param_1)

{
  ...
  plVar2 = (long *)param_1[0x22];
  lVar3 = param_1[0x23];
  plVar4 = (long *)plVar1[9];
  if (plVar2 == (long *)0x0) {
    param_1[0x24] = 2;
LAB_0010636f:
    lVar9 = plVar1[0xb];
...
LAB_00106396:
    ...
    *param_1 = (long)plVar7;
    param_1[10] = plVar1[0x14];
    if (plVar4 == (long *)0x0) {
      iVar6 = lzma_vli_size(0);
      param_1[9] = 0;
      param_1[8] = ((ulong)(iVar6 + 1) + 7 & 0xfffffffffffffffc) + 0x18;
      goto LAB_001063fc;
    }
  }
  else {
    if ((*(long **)(param_1[0x20] + 0x10) != plVar1) || (plVar2 != plVar4)) {
      param_1[0x24] = 0;
      goto LAB_0010636f;
    }
    if ((long *)plVar1[8] == plVar2) {
      param_1[0x24] = 2;
      param_1[0x22] = 0;
    }
    else {
      param_1[0x24] = 1;
      param_1[0x22] = plVar2[2];
    }
    lVar9 = plVar1[0xb];
    lVar11 = plVar1[1];
    lVar10 = *plVar1;
    ...
    param_1[6] = lVar11;
    param_1[7] = lVar10;
    if ((int)lVar5 != -1) goto LAB_00106396;
    lVar9 = plVar1[0x14];
    *param_1 = 0;
    param_1[10] = lVar9;
  }
  iVar6 = lzma_vli_size();
  lVar9 = plVar1[0xc];
  lVar5 = plVar4[plVar4[7] * 2 + 9];
  param_1[9] = plVar4[plVar4[7] * 2 + 8];
  param_1[8] = (lVar9 + 7 + (ulong)(iVar6 + 1) & 0xfffffffffffffffc) + 0x18 +
               (lVar5 + 3U & 0xfffffffffffffffc);
LAB_001063fc:
  if (plVar2 != (long *)0x0) {
    lVar9 = plVar2[5];
    param_1[0x12] = lVar9 + lVar3;
    param_1[0xf] = lVar9 + lVar3 + plVar1[6];
    if (lVar3 == 0) {
      uVar8 = plVar2[1];
      lVar9 = *plVar2;
    }
    else {
      lVar9 = plVar2[lVar3 * 2 + 6];
      uVar8 = plVar2[lVar3 * 2 + 7] + 3U & 0xfffffffffffffffc;
    }
    param_1[0x14] = lVar9;
    param_1[0x15] = plVar2[lVar3 * 2 + 8] - lVar9;
    lVar3 = plVar2[lVar3 * 2 + 9];
    param_1[0x11] = lVar10 + lVar9;
    param_1[0x16] = lVar3 - uVar8;
    param_1[0x13] = uVar8 + 0xc;
    param_1[0x17] = (lVar3 - uVar8) + 3 & 0xfffffffffffffffc;
    param_1[0x10] = uVar8 + 0xc + lVar11;
  }
  return;
}
...
/**
 * @name  lzma_index_tree_find_right_ancestor
 * @brief Traverses binary tree upward to find ancestor via right-child parent chain
 * @confidence 75%
 * @classification utility
 * @address 0x00106580
 */

/* Traverses binary tree upward to find ancestor via right-child parent chain */

void lzma_index_tree_find_right_ancestor(long param_1)
...
/* Sets uncompressed size limit on index, clamped to max VLI value */
 ...
 * @brief Calculates memory needed for index structure based on stream and record counts
 * @confidence 85%
 * @classification utility
 * @address 0x00106670
 */

/* Calculates memory needed for index: per-stream 0x128 bytes + per-block-group 0x2060 + 0x70
   overhead */
...
/* Calculates total LZMA index section size with VLI encoding and 4-byte alignment */
...
/* Calculates total XZ stream size: 24 bytes headers + blocks + aligned index size */
 ...
 * @brief Calculates total file size from index: sums stream data, headers/footers, index records, padding
 * @confidence 88%
 * @classification utility
 * @address 0x00106770
 */

/* Calculates total file size from index: sums stream data, headers/footers, index records, padding
    */
...
/* Returns bitmask of all integrity check types used in the index */
...
/* Calculates padding needed for 4-byte alignment of index data */
...
/* Sets stream flags for an LZMA index: validates flags via comparison, copies 7 qwords to internal
   structure */
...
/* Sets stream padding in index, validates 4-byte alignment and checks for overflow */
 ...
 * @brief Appends a new record to LZMA index. Validates sizes, allocates index groups with capacity doubling, updates counters.
 * @confidence 90%
 * @classification utility
 * @address 0x00106940
 */

/* Appends a new record to LZMA index. Validates sizes, computes padded sizes, checks overflow,
   allocates new index groups with capacity doubling, updates cumulative size counters and record
   count. */
    ...
    if ((uVar8 + 7 + lVar9 + (ulong)(iVar3 + 1) & 0xfffffffffffffffc) < 0x400000001) {
      if ((plVar7 == (long *)0x0) || (uVar6 = plVar7[7] + 1, (ulong)plVar7[6] <= uVar6)) {
        ...
        *plVar7 = local_50;
        ...
        lzma_index_tree_append(lVar1 + 0x38,plVar7);
        lVar2 = *(long *)(param_1 + 0x30);
        uVar6 = plVar7[7];
        lVar9 = *(long *)(param_1 + 0x38);
        lVar10 = *(long *)(lVar1 + 0x58) + 1;
...
/**
 * @name  lzma_index_cat
 * @brief Concatenates two LZMA index structures. Validates combined sizes, merges trees, accumulates statistics, frees source.
 * @confidence 88%
 * @classification utility
 * @address 0x00106c50
 */

/* Concatenates two LZMA index structures. Validates combined sizes, potentially reallocates last
   group, accumulates statistics from source tree, merges counters and check types, frees source. */
...
/**
 * @name  lzma_index_dup
 * @brief Deep-copies an LZMA index structure: duplicates tree of streams and groups with all record data.
 * @confidence 88%
 * @classification utility
 * @address 0x00106ea0
 */

/* Deep-copies an LZMA index structure. Duplicates the tree of streams and groups, copying all
   record data and statistics. Returns new index or 0 on failure. */
  ...
  if (local_50 != 0) {
    ...
    *(undefined8 *)(local_50 + 0x38) = *(undefined8 *)(param_1 + 0x38);
    ...
    while (uVar1 < 0xffffffffffffffc) {
        ...
        if (lVar8 != 0) {
          ...
          *puVar7 = 0;
          ...
          do {
            ...
            lVar8 = lzma_index_tree_find_right_ancestor(lVar8);
          } while (lVar8 != 0);
          lzma_index_tree_append(lVar6 + 0x38,puVar7);
        }
        lzma_index_tree_append(local_50,lVar6);
      ...
      puVar9 = (undefined8 *)lzma_index_tree_find_right_ancestor(puVar9);
...
/* Resets iterator state by zeroing 4 fields at offsets 0x108-0x120 */
 ...
 * @brief Advances LZMA index iterator. Mode: 0=any, 1=stream, 2=block, 3=non-empty block. Returns 0/1.
 * @confidence 88%
 * @classification utility
 * @address 0x00107130
 */

/* Advances LZMA index iterator to next element. Mode: 0=any, 1=stream, 2=block, 3=non-empty block.
   Returns 0 on success, 1 when exhausted. Traverses binary tree of index nodes. */
...

{
  ...
  if (lVar1 == 1) {
    ...
    if (plVar2 == (long *)0x0) {
      plVar5 = (long *)lzma_index_tree_find_right_ancestor();
      goto LAB_001071c4;
    }
    do {
      plVar5 = plVar2;
    ...
    if (plVar5 == (long *)0x0) {
      do {
        ...
        if (*(long *)(lVar4 + 0x20) == 0) {
          lVar4 = lzma_index_tree_find_right_ancestor(lVar4);
          if (lVar4 == 0) {
            return 1;
          }
          plVar5 = *(long **)(lVar4 + 0x40);
 ...
 * @brief Locates a target offset in LZMA index using two-level binary tree search and binary search within record groups.
 * @confidence 90%
 * @classification utility
 * @address 0x00107340
 */

/* Locates a target offset in an LZMA index using two-level binary tree search and binary search
   within records. */
...
/* Compares two LZMA stream flag structures: validates presets, compares check types and backward
   sizes */
 ...
 * @brief Formats error message using vsnprintf into global buffer at DAT_001323c0 (max 511 chars). Variadic function.
 * @confidence 88%
 * @classification utility
 * @address 0x00108880
 */

/* Formats error message using vsnprintf into global buffer at DAT_001323c0 (max 511 chars). */
...
/* Reads /proc/self/maps to find memory region containing address, returns permission bitmask (1=r,
   2=w, 4=x). */
...
/* Retrieves dynamic library link map via dlinfo() and parses ELF dynamic section */
...
/* Resolves dynamic symbol info from a code address using dladdr1, then parses the library's ELF
   dynamic section */
...
/* Counts bytes needed for variable-length integer encoding (7 bits per byte) */
...
/* Checks if output queue has data available for reading */
...
/* Calculates maximum compressed size with 3-byte overhead per 64K block plus 1 */
...
/* Internal block buffer bound calculation, same logic as public version */
...
/* Returns maximum buffer size for LZMA block compression including headers */
...
/* Calculates block header size by summing VLI sizes, filter flag sizes, and padding to 4-byte
   alignment. */
...
/* Single-call easy buffer encoding: builds preset filters, delegates to lzma_stream_buffer_encode
    */
...
/**
 * @name  lzma_easy_encoder_memusage
 * @brief Calculates memory usage for easy encoder at given preset
 * @confidence 92%
 * @classification utility
 * @address 0x0010b000
 */

/* Calculates memory usage for easy encoder at given preset */

undefined8 lzma_easy_encoder_memusage(undefined4 param_1)

{
  ...
  local_10 = *(long *)(in_FS_OFFSET + 0x28);
  cVar1 = lzma_easy_preset(auStack_d8,param_1);
  uVar2 = 0xffffffff;
  if (cVar1 == '\0') {
    uVar2 = lzma_raw_encoder_memusage(auStack_d8);
  }
  if (local_10 == *(long *)(in_FS_OFFSET + 0x28)) {
    return uVar2;
  }
...
/* Searches encoder filter table (12 entries) for matching filter ID, returns descriptor */
...
/* Checks if filter ID exists in encoder filter table (12 entries) */
...
/* Queries memory usage for raw LZMA encoding using encoder filter lookup */

void lzma_raw_encoder_memusage(undefined8 param_1)

{
  lzma_raw_coder_memusage(lzma_encoder_find_by_id,param_1);
  return;
}
...
/* Updates filter chain: validates memory usage, reverses filter array, calls coder's update
   function. */
...
/* Computes MT block size by iterating filters and finding the maximum recommended block size */
...
/* Determines property size for an LZMA filter by searching filter table and calling size handler or
   returning default */
...
/* Calculates encoded size of filter flags including VLI-encoded ID and properties size */
 ...
 * @brief Returns maximum output size for stream buffer compression (block bound + 0x30)
 * @confidence 85%
 * @classification utility
 * @address 0x0010bc30
 */

/* Returns maximum output size for stream buffer compression (block bound + 0x30 for headers/footer)
    */
...
/* Collects progress from MT encoder by summing per-worker in/out byte counts under mutex protection
    */

void lzma_stream_encoder_mt_get_progress(long param_1,long *param_2,long *param_3)
...
/* Validates MT block parameters: checks bounds, optionally initializes preset, calculates block and
   buffer sizes */
 ...
 * @brief Calculates memory usage for LZMA multi-threaded stream encoding by validating params and accumulating per-thread encoder, output queue, and block buffer costs.
 * @confidence 90%
 * @classification utility
 * @address 0x0010e2b0
 */

/* Calculates the memory usage required for LZMA multi-threaded stream encoding. Validates MT block
   parameters and accumulates memory requirements from various components. */

long lzma_stream_encoder_mt_memusage(long param_1)

{
  ...
  local_30 = *(long *)(in_FS_OFFSET + 0x28);
  iVar2 = lzma_mt_block_params_validate(param_1,local_f8,&local_110,&local_108,&local_100);
  if (iVar2 == 0) {
    lVar3 = lzma_raw_encoder_memusage(local_110);
    if (lVar3 != -1) {
      uVar1 = *(uint *)(param_1 + 4);
      uVar4 = lzma_outq_memusage(local_100,uVar1);
      if (uVar4 != 0xffffffffffffffff) {
        uVar5 = local_108 * (ulong)uVar1;
        uVar6 = (ulong)*(uint *)(param_1 + 4) * 0x220 + 0x8238;
        if (uVar5 <= ~uVar6) {
          uVar6 = uVar6 + uVar5;
          uVar5 = (ulong)uVar1 * lVar3;
          if (uVar5 <= ~uVar6) {
            uVar5 = uVar5 + uVar6;
            if (uVar4 <= ~uVar5) {
              lVar3 = uVar4 + uVar5;
...
/* Gets/sets LZMA2 decoder memory limit at offsets 0x68 and 0x70 */

undefined8
lzma_lzma2_decoder_memconfig(long param_1,undefined8 *param_2,undefined8 *param_3,ulong param_4)
...
/* Delegates get_check to sub-coder via function pointer at coder[6] */
 ...
 * @brief Calculates memory usage for easy decoder at given preset
 * @confidence 92%
 * @classification utility
 * @address 0x0010f7a0
 */

/* Calculates memory usage for easy decoder at given preset */

undefined8 lzma_easy_decoder_memusage(undefined4 param_1)

{
  ...
  uVar2 = 0xffffffff;
  if (cVar1 == '\0') {
    uVar2 = lzma_raw_decoder_memusage(auStack_d8);
  }
  if (local_10 == *(long *)(in_FS_OFFSET + 0x28)) {
    return uVar2;
  }
...
/* Manages LZ encoder buffer positioning: validates bounds, updates read position, handles buffer
   wraparound */

undefined8 lzma_lz_encoder_prepare(long param_1,long param_2,long *param_3,long param_4)
...
/* Calculates decoder memory usage including index, validates against limit, optionally updates
   limit. */

undefined8
lzma_file_info_decoder_memconfig(int *param_1,ulong *param_2,undefined8 *param_3,ulong param_4)
...
/* Searches decoder filter table (12 entries) for matching filter ID, returns descriptor */
...
/* Checks if filter ID exists in decoder filter table (12 entries) */
...
/* Queries memory usage for raw LZMA decoding using decoder filter lookup */

void lzma_raw_decoder_memusage(undefined8 param_1)

{
  lzma_raw_coder_memusage(lzma_decoder_find_by_id,param_1);
  return;
}
 ...
 * @brief Returns EBP register value, compiler-generated stack unwinding helper
 * @confidence 40%
 * @classification utility
 * @address 0x00110987
 */

/* Returns EBP register value, compiler-generated helper for stack unwinding */
...
/* Appends block info to index hash, validates sizes, updates VLI block size counters */
...

{
  ...
  if ((((param_1 != (int *)0x0) && (*param_1 == 0)) && (param_2 - 5U < 0x7ffffffffffffff8)) &&
     (-1 < param_3)) {
    lzma_index_hash_record_update(param_1 + 2);
    lVar1 = *(long *)(param_1 + 2);
 ...
 * @brief Returns R11D register value, likely compiler-generated helper
 * @confidence 40%
 * @classification utility
 * @address 0x001111a1
 */

/* Returns R11D register value, likely compiler-generated helper */
...
/* Gets/sets stream decoder memory limit at offsets 0x168 and 0x170 */

undefined8
lzma_stream_decoder_memconfig(long param_1,undefined8 *param_2,undefined8 *param_3,ulong param_4)
...
/* Stack canary validation - calls __stack_chk_fail on mismatch */
...
/* Collects progress from MT decoder by summing per-worker in/out byte counts under mutex protection
    */

void lzma_stream_decoder_mt_get_progress(long param_1,long *param_2,long *param_3)
...
/* Gets/sets memory usage and limit from lzip decoder state */

undefined8
lzma_lzip_decoder_memconfig(long param_1,undefined8 *param_2,undefined8 *param_3,ulong param_4)
...
/* Stack canary verification with passthrough return value */
...
/* Checks if LZMA integrity check type is supported via lookup table */
...
/* Returns byte size of LZMA integrity check type from lookup table */
...
/* Delegates get_check to sub-coder if encoder is not in LZMA2 uncompressed mode */
...
/* Calculates encoder memory usage for a given preset by building temp options and calling size
   calculator */

long lzma_lzma_encoder_memusage_preset(undefined8 param_1)

{
  ...
  cVar1 = lzma_lz_encoder_mf_init(&uStack_88,0,param_1);
  lVar2 = -1;
  if (cVar1 == '\0') {
...
/* Checks if match finder type is supported: HC3(3), HC4(4), BT2(18), BT3(19), BT4(20) */
...
/**
 * @name  lz77_find_matches
 * @brief LZ77 hash chain match finder: searches for longest matching strings using 8-byte comparisons. Returns (length, distance) pairs.
 * @confidence 85%
 * @classification utility
 * @address 0x00116fc0
 */

/* LZ77 match finder that searches a hash chain for the longest matching string. param_1 is max
   match length, param_3 is current position pointer, param_6 is hash chain table, param_7 is
   current position in circular buffer, param_8 is window size. Uses 8-byte comparison with bit
   scanning to find match length efficiently. Returns pairs of (length, distance) in output array.
    */

uint * lz77_find_matches(uint param_1,int param_2,char *param_3,int param_4,int param_5,long param_6
...

{
  ...
  *(int *)(param_6 + (ulong)param_7 * 4) = param_4;
  uVar1 = param_2 - param_4;
  if ((param_5 != 0) && (uVar1 < param_8)) {
    puVar6 = param_9;
    do {
      param_5 = param_5 - 1;
      uVar3 = param_4 + (param_7 - param_2);
      pcVar5 = param_3 + -(ulong)uVar1;
      if (param_7 < uVar1) {
        uVar3 = uVar3 + param_8;
      }
      param_4 = *(int *)(param_6 + (ulong)uVar3 * 4);
      param_9 = puVar6;
      if ((pcVar5[param_10] == param_3[param_10]) && (*pcVar5 == *param_3)) {
        if (1 < param_1) {
          uVar3 = 1;
LAB_00117081:
          uVar2 = *(long *)(pcVar5 + uVar3) - *(long *)(param_3 + uVar3);
          if (uVar2 == 0) goto LAB_00117078;
          uVar4 = 0;
...
LAB_00117010:
      uVar1 = param_2 - param_4;
      puVar6 = param_9;
    } while (param_5 != 0 && uVar1 < param_8);
  }
  return param_9;
LAB_00117078:
  uVar3 = uVar3 + 8;
  if (param_1 <= uVar3) goto LAB_001170e4;
  goto LAB_00117081;
}
 ...
 * @brief Binary tree match finder: traverses binary tree for longest matches, updates tree nodes during search.
 * @confidence 85%
 * @classification utility
 * @address 0x00117100
 */

/* Binary tree based LZ77 match finder. Traverses binary tree to find longest matches in sliding
   window using 8-byte word comparisons with trailing zero counting for fast length computation.
   Updates tree nodes during search. */
...
/**
 * @name  bt_match_find_insert
 * @brief Binary tree match finder insertion. Maintains left/right child pointers during search, uses 8-byte comparisons.
 * @confidence 85%
 * @classification utility
 * @address 0x00117300
 */

/* Binary tree match finder for LZ compression. Inserts a new node into a binary tree indexed by
   string comparison, maintaining left/right child pointers. Tracks the longest match in each
   subtree direction. Uses 8-byte wide comparisons with bit scan for efficient string matching.
   param_6 is the tree node array with pairs of (left_child, right_child). */

void bt_match_find_insert
               (uint param_1,int param_2,long param_3,int param_4,int param_5,long param_6,
...

{
  ...
  lVar4 = (ulong)(param_7 * 2) * 4 + 4;
  uVar6 = param_2 - param_4;
  piVar11 = (int *)(param_6 + lVar4);
  piVar12 = (int *)(param_6 - 4 + lVar4);
  if ((uVar6 < param_8) && (param_5 != 0)) {
    uVar10 = 0;
    uVar5 = 0;
    do {
      param_5 = param_5 - 1;
      iVar3 = (param_7 - param_2) + param_4;
      if (param_7 < uVar6) {
        iVar3 = iVar3 + param_8;
      }
      piVar1 = (int *)(param_6 + (ulong)(uint)(iVar3 * 2) * 4);
      lVar4 = param_3 - (ulong)uVar6;
      uVar6 = uVar5;
      if (uVar10 <= uVar5) {
        uVar6 = uVar10;
      }
      bVar9 = *(byte *)(lVar4 + (ulong)uVar6);
      bVar7 = *(byte *)(param_3 + (ulong)uVar6);
      if (bVar9 == bVar7) {
        uVar6 = uVar6 + 1;
        while( true ) {
          if (param_1 <= uVar6) goto LAB_00117450;
          uVar8 = *(long *)(lVar4 + (ulong)uVar6) - *(long *)(param_3 + (ulong)uVar6);
          if (uVar8 != 0) break;
          uVar6 = uVar6 + 8;
        }
        uVar2 = 0;
...
/* Normalizes match finder hash tables: rotates counter, conditionally decrements hash chain values
   when counter wraps */

void lzma_mf_normalize(long param_1)
 ...
 * @brief Calculates best match distance in LZ77 by comparing data bytes at offsets using 8-byte word comparisons.
 * @confidence 82%
 * @classification utility
 * @address 0x00117510
 */

/* Calculates best match distance in LZ77 by comparing data bytes at offsets and computing bit
   differences. */

ulong lzma_mf_find_best_match(long *param_1,int *param_2,long param_3)
...
/* HC3 match finder: computes 2-byte and 3-byte hashes, searches hash chains, returns matches found.
    */

ulong lzma_mf_hc3_find(long *param_1,uint *param_2)
...
/* HC3 hash chain skip: updates hash entries for count positions without searching for matches */

void lzma_mf_hc3_skip(long *param_1,int param_2)
 ...
 * @brief Fast hash chain match finder using 2/3/4-byte hash tables. Updates hash entries, checks short matches, delegates longer to lz77_find_matches.
 * @confidence 88%
 * @classification utility
 * @address 0x001177c0
 */

/* Fast hash chain match finder using 2/3/4-byte hash tables. Updates hash entries, checks short
   matches via hash2/hash3, delegates longer matches to lz77_find_matches. Returns match count. */

ulong lzma_mf_hc4_find(long *param_1,uint *param_2)
...
/* BT4 match finder skip: processes positions without finding matches, updates hash chains */

void lzma_mf_bt4_skip(long *param_1,int param_2)
...
/**
 * @name  lzma_mf_hc3_find
 * @brief HC3 hash chain match finder: updates hash tables and binary tree nodes, returns match count
 * @confidence 82%
 * @classification utility
 * @address 0x00117ab0
 */

/* HC3 hash chain match finder: updates hash tables and binary tree nodes, returns match count */

ulong lzma_mf_hc3_find(long *param_1,long param_2)
 ...
 * @brief BT3 match finder: iterates hash chain with binary tree search, returns match count
 * @confidence 82%
 * @classification utility
 * @address 0x00117b50
 */

/* BT3 match finder: iterates hash chain with binary tree search, returns match count */

uint lzma_mf_bt3_find(long *param_1,int param_2)

{
  ...
  do {
    while( true ) {
      ...
      uVar7 = uVar4;
      if ((uVar4 <= uVar9) || ((1 < uVar9 && (uVar7 = uVar9, (int)param_1[0xd] != 1)))) break;
      *(int *)(param_1 + 5) = (int)param_1[5] + 1;
    ...
    lVar6 = param_1[9];
    piVar1 = (int *)(param_1[8] + (ulong)*(ushort *)((ulong)uVar3 + lVar5) * 4);
    iVar2 = *piVar1;
    *piVar1 = iVar8;
    uVar4 = *(uint *)(param_1 + 10);
    bt_match_find_insert
              (uVar7,iVar8,(ushort *)((ulong)uVar3 + lVar5),iVar2,
               *(undefined4 *)((long)param_1 + 0x5c),lVar6);
    lzma_mf_normalize(param_1);
    param_2 = param_2 - 1;
  } while (param_2 != 0);
  return uVar4;
...
/**
 * @name  lzma_mf_hc3_find_bt
 * @brief HC3 match finder with binary tree: hash chain for short matches plus BT for longer, returns match count.
 * @confidence 80%
 * @classification utility
 * @address 0x00117be0
 */

/* HC3 (Hash Chain with 3-byte minimum) match finder. Computes hash from 3 bytes, updates hash
   chains, searches for matches by comparing bytes at match distance, and returns number of matches
   found. Uses XOR-based byte comparison to quickly find match length. */
...

{
  ...
  uVar5 = *(int *)((long)param_1 + 0x24) - uVar8;
  uVar4 = *(uint *)(param_1 + 0xc);
  if ((uVar5 < *(uint *)(param_1 + 0xc)) && ((uVar5 < 3 || (uVar4 = uVar5, (int)param_1[0xd] == 1)))
     ) {
    *(int *)(param_1 + 5) = (int)param_1[5] + 1;
    *(uint *)(param_1 + 3) = uVar8 + 1;
    return 0;
  }
  pbVar10 = (byte *)((ulong)uVar8 + *param_1);
  iVar11 = uVar8 + *(int *)((long)param_1 + 0x14);
  lVar6 = param_1[8];
  bVar2 = pbVar10[1];
  uVar8 = *(uint *)(&crc32_lookup_table + (ulong)*pbVar10 * 4);
  piVar1 = (int *)(lVar6 + (ulong)((bVar2 ^ uVar8) & 0x3ff) * 4);
  bVar3 = pbVar10[2];
  uVar12 = iVar11 - *piVar1;
  uVar5 = *(uint *)(param_1 + 0xb);
  *piVar1 = iVar11;
  *(int *)(lVar6 + (ulong)((((uint)bVar3 << 8 ^ bVar2 ^ uVar8) & uVar5) + 0x400) * 4) = iVar11;
  if (uVar12 < *(uint *)((long)param_1 + 0x54)) {
    uVar9 = 2;
    if (pbVar10[-(ulong)uVar12] == *pbVar10) {
      if (2 < uVar4) {
LAB_00117d07:
        uVar7 = *(long *)(pbVar10 + uVar9) - *(long *)(pbVar10 + (uVar9 - uVar12));
        if (uVar7 == 0) goto LAB_00117d00;
        uVar8 = 0;
        for (; (uVar7 & 1) == 0; uVar7 = uVar7 >> 1 | 0x8000000000000000) {
          uVar8 = uVar8 + 1;
        }
        uVar5 = (uVar8 >> 3) + (int)uVar9;
        uVar8 = uVar5;
        if (uVar4 <= uVar5) {
          uVar8 = uVar4;
        }
        param_2[1] = uVar12 - 1;
        *param_2 = uVar8;
        if (uVar4 <= uVar5) goto LAB_00117d52;
        goto LAB_00117c85;
      }
LAB_00117d48:
      *param_2 = uVar4;
...
LAB_00117c85:
  lVar6 = lz_find_matches_bt();
  uVar9 = lVar6 - (long)param_2 >> 3;
  lzma_mf_normalize(param_1);
LAB_00117cb3:
  return uVar9 & 0xffffffff;
LAB_00117d00:
  uVar8 = (int)uVar9 + 8;
  uVar9 = (ulong)uVar8;
  if (uVar4 <= uVar8) goto LAB_00117d48;
  goto LAB_00117d07;
}
 ...
 * @brief HC4 hash chain match finder skip operation for multiple positions.
 * @confidence 85%
 * @classification utility
 * @address 0x00117d80
 */

/* HC3 hash chain match finder with combined find and skip operation for count positions. */

uint lzma_mf_hc4_skip(long *param_1,int param_2)
 ...
 * @brief Optimal binary tree match finder for LZMA with 4-level hash. Includes mode check and delegates to bt_match_find_insert.
 * @confidence 88%
 * @classification utility
 * @address 0x00117e50
 */

/* Optimal binary tree match finder for LZMA. Similar to fast variant but includes mode check for
   limited bytes and calls bt_match_find_insert for hash chain updates when matches reach nice
   length. */

ulong lzma_mf_bt4_find(long *param_1,uint *param_2)
 ...
 * @brief BT4 match finder skip operation: updates 4-level hash chains and binary tree for multiple positions without returning matches.
 * @confidence 85%
 * @classification utility
 * @address 0x001180a0
 */

/* BT4 (Binary Tree with 4-byte hash) match finder: updates 4-level hash chains, searches for
   matches using binary tree. */

uint lzma_mf_bt4_find_and_skip(long *param_1,int param_2)
...
/* Sets LZMA2 encoder output limit (must be >5), stores limit and memusage pointer */
...
/* Validates LZMA options: lc<5, lp<5, lc+lp<5, pb<5, mode 1-2, nice_len 2-273 */
...
/* Calculates LZMA2 encoder memory usage from options. Validates lclppb, builds internal params,
   adds 0x3ced0 overhead */

long lzma_lzma2_encoder_memusage(uint *param_1)

{
  ...
  if (cVar2 != '\0') {
    ...
    local_20 = param_1[4];
    lVar3 = lzma_lzma_encoder_memusage_preset(&local_58);
    if (lVar3 != -1) {
      lVar3 = lVar3 + 0x3ced0;
      goto LAB_0011a7fa;
    }
 ...
 * @brief Core LZMA encoder function: checks rep0-rep3 distances, evaluates normal matches, applies heuristics. Returns len=1/dist=-1 for literal.
 * @confidence 85%
 * @classification utility
 * @address 0x0011a8d0
 */

/* Core LZMA encoder function finding optimal match or literal. Checks rep0-rep3 distances first,
   evaluates normal matches from match finder, applies heuristics. Returns length=1/dist=-1 for
   literal. */

void lzma_lzma_find_match_or_literal(long param_1,long *param_2,int *param_3,uint *param_4)

{
  ...
  local_40 = *(long *)(in_FS_OFFSET + 0x28);
  if (*(int *)((long)param_2 + 0x1c) == 0) {
    uVar3 = lzma_mf_find_best_match(param_2,&local_44,param_1 + 0x2dc);
  }
  else {
...
LAB_0011a9ff:
    ...
    if ((1 < uVar3) && (2 < uVar12)) {
      uVar12 = lzma_mf_find_best_match(param_2,param_1 + 0xb6c,(uint *)(param_1 + 0x2dc));
...
/* Sets output size limit and flag on LZMA encoder state */
...
/* Stores encoding state values to large encoder structure at fixed offsets. Sets error state
   conditionally. */

void lzma_lzma_encoder_store_state
...
/* Computes LZMA encoder memory usage as match_finder_memusage + 0x6ec0 */

long lzma_lzma_encoder_memusage(undefined4 *param_1)
...
/**
 * @name  lzma_lzma_lclppb_decoder_memusage
...
/* Validates lc/lp/pb LZMA parameters then computes decoder memory usage */

long lzma_lzma_lclppb_decoder_memusage(undefined4 *param_1)
 ...
 * @brief Computes LZMA2 decoder memory usage with 0x100a8 overhead
 * @confidence 85%
 * @classification utility
 * @address 0x00120d00
 */

/* Computes LZMA2 decoder memory usage with 0x100a8 overhead added */

long lzma_lzma2_decoder_memusage(void)

{
  long lVar1;
  
  lVar1 = lzma_lzma2_encoder_memusage();
  if (lVar1 != -1) {
    lVar1 = lVar1 + 0x100a8;
  }
  return lVar1;
...
/* Calculates decoder memory from dict size (3x), minimum 1MB, validates dict range */
...
/**
 * @name  lzma_lzma2_encoder_memusage
 * @brief Computes LZMA2 encoder memory usage as LZMA1 memusage + 0xb8
 * @confidence 85%
 * @classification utility
 * @address 0x00121260
 */

/* Computes LZMA2 encoder memory usage as LZMA1 memusage + 0xb8 for wrapper state */
...

{
  ...
  lVar1 = lzma_lzma_encoder_memusage();
  return lVar1 + 0xb8;
}
 ...
 * @brief Validates simple filter properties: checks null, marker, and start_offset range
 * @confidence 80%
 * @classification utility
 * @address 0x00121470
 */

/* Validates simple filter properties: checks null, marker, and start_offset range */
...
/* Sets output size to 0 or 4 based on whether delta options are present */

undefined8 lzma_delta_props_size(undefined4 *param_1,int *param_2)
